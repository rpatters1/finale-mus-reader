...
public:
    ...
    /// MIDI note.
    [[nodiscard]] std::optional<musx::dom::PercussionNoteTypeId> find(std::string_view mapName, std::uint16_t midiNote) const;

private:
    std::unordered_map<std::string, MappingTable> m_tables;

    friend MappingTables parseMappingTables(std::span<const std::span<const std::uint8_t>>, XmlParser);
};
...
[[nodiscard]] MappingTables parseMappingTables(std::span<const std::span<const std::uint8_t>> documents, XmlParser parseXml);
