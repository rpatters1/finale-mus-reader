...
std::optional<NoteType> MappingTables::find(std::string_view mapName, std::uint16_t midiNote) const
{
...
MappingTables parseMappingTables(std::span<const std::span<const std::uint8_t>> documents, XmlParser parseXml)
{
    ...
    for (const auto document : documents) {
        ...
        const auto xml = parseXml(reinterpret_cast<const char*>(document.data()), document.size());
