...
ImportReport staffImport(const finale_mus_reader::container::ParsedContainer& parsed, const SourceProfile& profile,
...
TEST_CASE("Staff name references compare through their block text", "[coverage]")
{
    ...
    const auto makeDocument = [](musx::dom::Cmper blockId, musx::dom::Cmper textId, std::string text) {
        auto session = musx::factory::DocumentFactory::begin();
        const auto document = session.getDocument();
        auto raw = std::make_shared<BlockText>(document, musx::dom::SCORE_PARTID, musx::dom::EnigmaBase::ShareMode::All, textId);
        raw->text = std::move(text);
        document->getTexts()->add(BlockText::XmlNodeName, std::move(raw));
        auto block = std::make_shared<TextBlock>(document, musx::dom::SCORE_PARTID, musx::dom::EnigmaBase::ShareMode::All, blockId);
        block->textId = textId;
    ...
    auto danglingBlock = std::make_shared<TextBlock>(dangling, musx::dom::SCORE_PARTID, musx::dom::EnigmaBase::ShareMode::All, musx::dom::Cmper(29));
    danglingBlock->textId = 53;
    dangling->getOthers()->add(TextBlock::XmlNodeName, std::move(danglingBlock));
    REQUIRE(comparison_text::compareStaffNameReferents("staff[cmper=1].full_name_text_id", 7, 19, source, equivalent) == true);
    REQUIRE(comparison_text::compareStaffNameReferents("staff_style[cmper=1].full_name_text_id", 7, 19, source, equivalent) == true);
    REQUIRE(comparison_text::compareStaffNameReferents("staff[cmper=1].abbrv_name_text_id", 7, 7, source, different) == false);
    REQUIRE(comparison_text::compareStaffNameReferents("staff_style[cmper=1].abbrv_name_text_id", 7, 7, source, different) == false);
    REQUIRE(comparison_text::compareStaffNameReferents("staff[cmper=1].full_name_text_id", 0, 29, source, dangling) == true);
    REQUIRE(comparison_text::compareStaffNameReferents("staff[cmper=1].abbrv_name_text_id", 23, 0, formattingOnly, equivalent) == true);
    REQUIRE(comparison_text::compareStaffNameReferents("staff[cmper=1].full_name_text_id", 0, 19, source, equivalent) == false);
    REQUIRE_FALSE(comparison_text::compareStaffNameReferents("staff[cmper=1].default_clef", 7, 19, source, equivalent));

    const auto snapshot = [](std::int64_t fullName, std::int64_t abbreviatedName) {
        return SurveySnapshot{
            {"staff", Value::Array{Value::Object{{"cmper", 1}, {"full_name_text_id", fullName}, {"abbrv_name_text_id", abbreviatedName}}}}};
    };
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::CodaBanner);
    const auto zeroMismatch = compareSnapshots(snapshot(7, 0), snapshot(19, 7), source, equivalent, finale_mus_reader::FormatEpoch::CodaBanner,
    ...
    CHECK(zeroMismatch.transformations.at(ComparisonTransformation::EquivalentTextBlockReferent) == 2);

    const auto equalComparatorDifferentText = compareSnapshots(snapshot(7, 0), snapshot(7, 0), source, different,
        finale_mus_reader::FormatEpoch::CodaBanner, finale_mus_reader::ByteOrder::BigEndian, nullptr, report);
...
TEST_CASE("Disabled Staff note-font size differences are different defaults", "[coverage]")
{
    ...
    const auto context = [&](const ComparisonLeaves& source, std::int64_t sourceSize, std::int64_t companionSize, std::string_view origin,
                             DifferenceCategory category = DifferenceCategory::Differs) {
        return DifferenceContext{"staff[cmper=1].note_font.font_size", category, origin, Value(sourceSize), Value(companionSize), source, none,
            finale_mus_reader::FormatEpoch::ZlibLegacy, finale_mus_reader::ByteOrder::LittleEndian, nullptr, report};
    };

    REQUIRE(classifyStaffDifference(context(disabled, 26, 0, "legacy-mus")) == DifferenceClassification::DifferentDefaults);
    REQUIRE(classifyStaffDifference(context(disabled, 0, 24, "finale27-default")) == DifferenceClassification::DifferentDefaults);
    REQUIRE_FALSE(classifyStaffDifference(context(enabled, 26, 0, "legacy-mus")));
    REQUIRE_FALSE(classifyStaffDifference(context(disabled, 26, 0, "legacy-mus", DifferenceCategory::ReaderOnly)));
}
    ...
    "[coverage]")
{
    ...
    const finale_mus_reader::SourceVersion finale2008{.major = finale_mus_reader::versions::finale2008.major};
    const finale_mus_reader::SourceVersion finale2011{.major = finale_mus_reader::versions::finale2011.major};
    const finale_mus_reader::SourceVersion finale2012{.major = finale_mus_reader::versions::finale2012.major};
    const finale_mus_reader::SourceVersion finale97{.major = finale_mus_reader::versions::finale97.major};
    const finale_mus_reader::SourceVersion finale2005{.major = finale_mus_reader::versions::finale2005.major};
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::ZlibLegacy);
    const ComparisonLeaves percussion{{"staff[cmper=1].notation_style", {Value(static_cast<std::int64_t>(NotationStyle::Percussion)), "legacy-mus"}}};
    const ComparisonLeaves tablature{{"staff[cmper=1].notation_style", {Value(static_cast<std::int64_t>(NotationStyle::Tablature)), "legacy-mus"}}};
    const ComparisonLeaves standard{{"staff[cmper=1].notation_style", {Value(static_cast<std::int64_t>(NotationStyle::Standard)), "legacy-mus"}}};
    const ComparisonLeaves none;
    const auto context = [&](const ComparisonLeaves& leaves, const Value& source, const Value& companion, std::string_view origin,
                             DifferenceCategory category, finale_mus_reader::FormatEpoch epoch, const finale_mus_reader::SourceVersion* version) {
        return DifferenceContext{"staff[cmper=1].use_note_font", category, origin, source, companion, leaves, none, epoch,
            finale_mus_reader::ByteOrder::BigEndian, version, report};
    };

    REQUIRE(classifyStaffDifference(
                context(percussion, no, yes, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2008))
            == DifferenceClassification::FinaleUpgradeLoss);
    REQUIRE(classifyStaffDifference(context(
                percussion, no, yes, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::UncompressedLegacy, &finale97))
            == DifferenceClassification::FinaleUpgradeLoss);
    REQUIRE(classifyStaffDifference(
                context(tablature, no, yes, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::DclLegacy, &finale2005))
            == DifferenceClassification::FinaleUpgradeLoss);
    REQUIRE_FALSE(classifyStaffDifference(
        context(standard, no, yes, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2008)));
    REQUIRE(classifyStaffDifference(
                context(percussion, yes, no, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2008))
            == DifferenceClassification::FinaleUpgradeLoss);
    REQUIRE(classifyStaffDifference(
                context(percussion, yes, no, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2011))
            == DifferenceClassification::FinaleUpgradeLoss);
    REQUIRE_FALSE(classifyStaffDifference(
        context(percussion, no, yes, "legacy-behavior", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2008)));
    REQUIRE_FALSE(classifyStaffDifference(
        context(percussion, no, yes, "legacy-mus", DifferenceCategory::ReaderOnly, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2008)));
    REQUIRE_FALSE(classifyStaffDifference(
        context(percussion, no, yes, "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2012)));
}
...
TEST_CASE("Coverage excludes the Studio View Staff", "[coverage]")
{
    ...
    const auto addStaff = [&](musx::dom::Cmper cmper) {
        auto staff = std::make_shared<Staff>(document, musx::dom::SCORE_PARTID, musx::dom::EnigmaBase::ShareMode::All, cmper);
...
TEST_CASE("Only pre-Finale 2012 behavioral Staff UUIDs have different defaults", "[coverage]")
{
    ...
    const finale_mus_reader::SourceVersion finale2011{.major = finale_mus_reader::versions::finale2011.major};
    const finale_mus_reader::SourceVersion finale2012{.major = finale_mus_reader::versions::finale2012.major};
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category,
                             const finale_mus_reader::SourceVersion* version) {
        return DifferenceContext{path, category, origin, source, companion, leaves, leaves, finale_mus_reader::FormatEpoch::ZlibLegacy,
            finale_mus_reader::ByteOrder::LittleEndian, version, report};
    };

    REQUIRE(classifyStaffDifference(context("staff[cmper=1].inst_uuid", "legacy-behavior", DifferenceCategory::Differs, &finale2011))
            == DifferenceClassification::DifferentDefaults);
    const DifferenceContext coda{"staff[cmper=1].inst_uuid", DifferenceCategory::Differs, "legacy-behavior", source, companion, leaves, leaves,
        finale_mus_reader::FormatEpoch::CodaBanner, finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
    REQUIRE(classifyStaffDifference(coda) == DifferenceClassification::DifferentDefaults);
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].inst_uuid", "legacy-mus", DifferenceCategory::Differs, &finale2011)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].inst_uuid", "legacy-behavior", DifferenceCategory::Differs, &finale2012)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].inst_uuid", "legacy-behavior", DifferenceCategory::ReaderOnly, &finale2011)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].line_space", "legacy-behavior", DifferenceCategory::Differs, &finale2011)));
}
    ...
    "[coverage]")
{
    ...
    const ComparisonLeaves leaves;
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::ZlibLegacy);
    const finale_mus_reader::SourceVersion finale2010{.major = finale_mus_reader::versions::finale2010.major};
    const finale_mus_reader::SourceVersion finale2011{.major = finale_mus_reader::versions::finale2011.major};
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category, const Value& source, const Value& companion,
                             finale_mus_reader::FormatEpoch epoch, const finale_mus_reader::SourceVersion* version) {
        return DifferenceContext{
            path, category, origin, source, companion, leaves, leaves, epoch, finale_mus_reader::ByteOrder::LittleEndian, version, report};
    };

    REQUIRE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::Differs, none, score,
                finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::Differs, cutaway, score,
                finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "finale27-default", DifferenceCategory::Differs, none, score,
                finale_mus_reader::FormatEpoch::CodaBanner, nullptr))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "finale27-default", DifferenceCategory::Differs, none, cutaway,
                finale_mus_reader::FormatEpoch::CodaBanner, nullptr))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "finale27-default", DifferenceCategory::Differs, none, score,
                finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2011))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::Differs, none, score,
        finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2011)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::Differs, cutaway, score,
        finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2011)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::Differs, none, cutaway,
        finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::Differs, score, none,
        finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "finale27-default", DifferenceCategory::Differs, score, none,
        finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010)));
    REQUIRE_FALSE(classifyStaffDifference(context(
        "staff[cmper=1].hide_mode", "unmapped", DifferenceCategory::Differs, none, score, finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_mode", "legacy-mus", DifferenceCategory::ReaderOnly, none, score,
        finale_mus_reader::FormatEpoch::ZlibLegacy, &finale2010)));
}
...
TEST_CASE("Finale 2012 beta Staff accidental display behavior is upgrade loss", "[coverage]")
{
    ...
    const ComparisonLeaves leaves;
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::ZlibLegacy);
    const finale_mus_reader::SourceVersion beta{.major = finale_mus_reader::versions::finale2012.major, .devStatus = 2};
    const finale_mus_reader::SourceVersion release{.major = finale_mus_reader::versions::finale2012.major};
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category, const Value& source, const Value& companion,
                             const finale_mus_reader::SourceVersion* version) {
        return DifferenceContext{path, category, origin, source, companion, leaves, leaves, finale_mus_reader::FormatEpoch::ZlibLegacy,
            finale_mus_reader::ByteOrder::LittleEndian, version, report};
    ...
    REQUIRE(classifyStaffDifference(
                context("staff[cmper=1].hide_key_sigs_show_accis", "legacy-behavior", DifferenceCategory::Differs, disabled, enabled, &beta))
            == DifferenceClassification::BetaDiscrepancy);
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].hide_key_sigs_show_accis", "legacy-behavior", DifferenceCategory::Differs, disabled, enabled, &release)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].hide_key_sigs_show_accis", "legacy-mus", DifferenceCategory::Differs, disabled, enabled, &beta)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].hide_key_sigs_show_accis", "legacy-behavior", DifferenceCategory::Differs, enabled, disabled, &beta)));
}
    ...
    "[coverage]")
{
    ...
    const ComparisonLeaves leaves;
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::CodaBanner);
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category, finale_mus_reader::FormatEpoch epoch) {
        return DifferenceContext{
            path, category, origin, source, companion, leaves, leaves, epoch, finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
    };

    const Value shown{true};
    const Value hidden{false};
    const auto hideKeySigs = [&](finale_mus_reader::FormatEpoch epoch) {
        return DifferenceContext{"staff[cmper=1].hide_key_sigs", DifferenceCategory::Differs, "unmapped", hidden, shown, leaves, leaves, epoch,
            finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
    };
    REQUIRE_FALSE(classifyStaffDifference(hideKeySigs(finale_mus_reader::FormatEpoch::UncompressedLegacy)));
    report.setField(
        finale_mus_reader::instanceKey<Staff>(musx::dom::SCORE_PARTID, staffCmper1), "dwRestOffset", {ValueOrigin::Finale27Default, 0, 0, -4});
    for (const auto epoch : {finale_mus_reader::FormatEpoch::CodaBanner, finale_mus_reader::FormatEpoch::UncompressedLegacy}) {
        REQUIRE(classifyStaffDifference(hideKeySigs(epoch)) == DifferenceClassification::PossiblyUnrecoverable);
    }

    for (const auto suffix : {"dw_rest_offset", "w_rest_offset", "h_rest_offset", "other_rest_offset", "stem_reversal"}) {
        for (const auto epoch : {finale_mus_reader::FormatEpoch::CodaBanner, finale_mus_reader::FormatEpoch::UncompressedLegacy}) {
            REQUIRE(classifyStaffDifference(context("staff[cmper=1]." + std::string(suffix), "finale27-default", DifferenceCategory::Differs, epoch))
                    == DifferenceClassification::PossiblyUnrecoverable);
        }
    }
    const Value zero{0};
    const Value two{2};
    const DifferenceContext fretInstrument{"staff[cmper=1].fret_inst_id", DifferenceCategory::Differs, "finale27-default", zero, two, leaves, leaves,
        finale_mus_reader::FormatEpoch::CodaBanner, finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
    REQUIRE(classifyStaffDifference(fretInstrument) == DifferenceClassification::PossiblyUnrecoverable);
    const DifferenceContext reverseFretInstrument{"staff[cmper=1].fret_inst_id", DifferenceCategory::Differs, "finale27-default", two, zero, leaves,
    ...
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].dw_rest_offset", "legacy-behavior", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::CodaBanner)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].dw_rest_offset", "finale27-default", DifferenceCategory::ReaderOnly, finale_mus_reader::FormatEpoch::CodaBanner)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("measures[cmper=1].dw_rest_offset", "finale27-default", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::CodaBanner)));
}
...
TEST_CASE("Synthesized Staff fret instrument references may be renumbered", "[coverage]")
{
    ...
    report.setInstanceOrigin(
        finale_mus_reader::instanceKey<FretInstrument>(musx::dom::SCORE_PARTID, musx::dom::Cmper{2}), ValueOrigin::LegacyBehavior);
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category, const Value& source,
                             const Value& companion) {
        return DifferenceContext{path, category, origin, source, companion, leaves, leaves, finale_mus_reader::FormatEpoch::DclLegacy,
            finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
    };

    REQUIRE(classifyStaffDifference(context("staff[cmper=1].fret_inst_id", "legacy-behavior", DifferenceCategory::Differs, Value{2}, Value{1}))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].fret_inst_id", "legacy-mus", DifferenceCategory::Differs, Value{2}, Value{1})));
    REQUIRE_FALSE(
        classifyStaffDifference(context("staff[cmper=1].fret_inst_id", "legacy-behavior", DifferenceCategory::Differs, Value{3}, Value{1})));
    REQUIRE_FALSE(
        classifyStaffDifference(context("staff[cmper=1].fret_inst_id", "legacy-behavior", DifferenceCategory::ReaderOnly, Value{2}, Value{1})));
    REQUIRE_FALSE(
        classifyStaffDifference(context("staff[cmper=1].notation_style", "legacy-behavior", DifferenceCategory::Differs, Value{2}, Value{1})));
}
...
TEST_CASE("Defaulted Staff tablature line breaking may differ", "[coverage]")
{
    ...
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::DclLegacy);
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category) {
        return DifferenceContext{path, category, origin, Value{false}, Value{true}, leaves, leaves, finale_mus_reader::FormatEpoch::DclLegacy,
            finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
    };

    REQUIRE(classifyStaffDifference(context("staff[cmper=1].break_tab_lines_at_notes", "finale27-default", DifferenceCategory::Differs))
            == DifferenceClassification::DifferentDefaults);
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].break_tab_lines_at_notes", "legacy-behavior", DifferenceCategory::Differs)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].break_tab_lines_at_notes", "finale27-default", DifferenceCategory::ReaderOnly)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=1].hide_beams", "finale27-default", DifferenceCategory::Differs)));
}
...
TEST_CASE("Only derived Staff styles are deferred", "[coverage]")
{
    ...
    const auto codaUnmapped =
        context("staff[cmper=1].dw_rest_offset", "unmapped", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::CodaBanner);
    REQUIRE_FALSE(classifyStaffDifference(codaUnmapped));
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].dw_rest_offset", "legacy-behavior", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::CodaBanner)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].dw_rest_offset", "unmapped", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::UncompressedLegacy)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].dw_rest_offset", "unmapped", DifferenceCategory::ReaderOnly, finale_mus_reader::FormatEpoch::CodaBanner)));
    REQUIRE_FALSE(classifyStaffDifference(
        context("measures[cmper=1].dw_rest_offset", "unmapped", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::CodaBanner)));

    const auto derivedStyles =
        context("staff[cmper=1].has_styles", "unmapped", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy);
    REQUIRE(classifyStaffDifference(derivedStyles) == DifferenceClassification::AwaitsDependentRecovery);
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].has_styles", "legacy-mus", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy)));
    const Value noStyles{false};
    const Value hasStyles{true};
    const finale_mus_reader::SourceVersion finale98{.major = finale_mus_reader::versions::finale98.major};
    const DifferenceContext preFinale2000HasStyles{"staff[cmper=1].has_styles", DifferenceCategory::Differs, "legacy-mus-adjusted", noStyles,
        hasStyles, leaves, leaves, finale_mus_reader::FormatEpoch::UncompressedLegacy, finale_mus_reader::ByteOrder::BigEndian, &finale98, report};
    REQUIRE(classifyStaffDifference(preFinale2000HasStyles) == DifferenceClassification::AwaitsDependentRecovery);
    REQUIRE_FALSE(classifyStaffDifference(
        context("staff[cmper=1].line_space", "unmapped", DifferenceCategory::Differs, finale_mus_reader::FormatEpoch::ZlibLegacy)));
...
TEST_CASE("Post-Finale-2000 verified assignment-source absence explains Staff hasStyles", "[coverage]")
{
    using namespace finale_mus_reader::coverage;
    const Value disabled(false);
    const Value enabled(true);
    ...
    const auto context = [&](const ImportReport& report, FormatEpoch epoch, const SourceVersion& version,
                             std::string_view path = "staff[cmper=1].has_styles") {
        return DifferenceContext{path, DifferenceCategory::Differs, "legacy-mus-adjusted", disabled, enabled, leaves, leaves, epoch,
            ByteOrder::BigEndian, &version, report};
    };

    ImportReport incompleteAudit(FormatEpoch::DclLegacy);
    CHECK_FALSE(classifyStaffDifference(context(incompleteAudit, FormatEpoch::DclLegacy, finale2006)));

    ImportReport verifiedAbsent(FormatEpoch::DclLegacy);
    verifiedAbsent.staffStyleAssignmentAuditComplete = true;
    REQUIRE(classifyStaffDifference(context(verifiedAbsent, FormatEpoch::DclLegacy, finale2006)) == DifferenceClassification::FinaleUpgradeSynthesis);
    ...
    CHECK_FALSE(classifyStaffDifference(context(sourcePresent, FormatEpoch::DclLegacy, finale2006)));
    ...
    CHECK_FALSE(classifyStaffDifference(context(malformedSource, FormatEpoch::DclLegacy, finale2006)));
    ...
    CHECK_FALSE(classifyStaffDifference(context(partSource, FormatEpoch::DclLegacy, finale2006, "staff[part_id=3,cmper=1].has_styles")));

    CHECK(classifyStaffDifference(context(verifiedAbsent, FormatEpoch::UncompressedLegacy, finale2000))
...
TEST_CASE("Legacy note-attached-items expansion loses fallback expression display", "[coverage]")
{
    ...
    for (const auto& [className, classPrefix, otherPrefix] : {
        ...
        ComparisonLeaves sourceLeaves{
            ...
            {objectPath + ".alt_notation", {Value(static_cast<std::int64_t>(Staff::AlternateNotation::Normal)), "legacy-mus"}},
        };
        ComparisonLeaves companionLeaves = sourceLeaves;
        const auto context = [&](const ComparisonLeaves& source, const ComparisonLeaves& companion, FormatEpoch epoch, const SourceVersion* version,
                                 const Value* sourceValue = nullptr, const Value* companionValue = nullptr) {
            return DifferenceContext{differencePath, DifferenceCategory::Differs, "finale27-default", sourceValue ? *sourceValue : disabled,
                companionValue ? *companionValue : enabled, source, companion, epoch, ByteOrder::BigEndian, version, report};
        };

        REQUIRE(classify(context(sourceLeaves, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000))
                == DifferenceClassification::FinaleUpgradeLoss);
        REQUIRE(classify(context(sourceLeaves, companionLeaves, FormatEpoch::DclLegacy, &finale2006)) == DifferenceClassification::FinaleUpgradeLoss);
        REQUIRE(
            classify(context(sourceLeaves, companionLeaves, FormatEpoch::ZlibLegacy, &finale2008)) == DifferenceClassification::FinaleUpgradeLoss);
        CHECK_FALSE(classify(context(sourceLeaves, companionLeaves, FormatEpoch::UncompressedLegacy, &finale98)));
        REQUIRE(
            classify(context(sourceLeaves, companionLeaves, FormatEpoch::ZlibLegacy, &finale2009)) == DifferenceClassification::FinaleUpgradeLoss);

        auto missingAggregateLeaf = sourceLeaves;
        missingAggregateLeaf.erase(objectPath + ".alt_hide_smart_shapes");
        CHECK_FALSE(classify(context(missingAggregateLeaf, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000)));
        auto wrongAggregateOrigin = sourceLeaves;
        wrongAggregateOrigin.at(objectPath + ".alt_hide_lyrics").second = "finale27-default";
        CHECK_FALSE(classify(context(wrongAggregateOrigin, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000)));
        auto disabledAggregateLeaf = sourceLeaves;
        disabledAggregateLeaf.at(objectPath + ".hide_chords").first = Value(false);
        CHECK_FALSE(classify(context(disabledAggregateLeaf, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000)));
        auto differingCompanionExpansion = companionLeaves;
        differingCompanionExpansion.at(objectPath + ".hide_chords").first = Value(false);
        CHECK(classify(context(sourceLeaves, differingCompanionExpansion, FormatEpoch::UncompressedLegacy, &finale2000))
              == DifferenceClassification::FinaleUpgradeLoss);
        auto rhythmicSource = sourceLeaves;
        rhythmicSource.at(objectPath + ".alt_notation").first = Value(static_cast<std::int64_t>(Staff::AlternateNotation::Rhythmic));
        CHECK(classify(context(rhythmicSource, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000))
              == DifferenceClassification::FinaleUpgradeLoss);
        CHECK_FALSE(classify(context(sourceLeaves, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000, &enabled, &disabled)));

        auto wrongClass = context(sourceLeaves, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000);
        ...
        auto wrongOrigin = context(sourceLeaves, companionLeaves, FormatEpoch::UncompressedLegacy, &finale2000);
    ...
    "[coverage]")
{
    ...
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::ZlibLegacy);
    const auto document = emptyStaffDocument();
    auto staffUsed = std::make_shared<musx::dom::others::StaffUsed>(document, musx::dom::SCORE_PARTID, musx::dom::EnigmaBase::ShareMode::All,
    ...
    const auto context = [&](std::string_view path, std::string_view origin, DifferenceCategory category) {
        return DifferenceContext{path, category, origin, source, companion, leaves, leaves, finale_mus_reader::FormatEpoch::ZlibLegacy,
            finale_mus_reader::ByteOrder::LittleEndian, nullptr, report, {}, {}, nullptr, nullptr, document.get()};
    };

    for (const auto suffix :
        {".full_name_text_id", ".abbrv_name_text_id", ".note_font.font_size", ".transposition.present", ".transposition.keysig.present",
            ".transposition.keysig.interval", ".transposition.keysig.adjust", ".hide_meas_nums", ".hide_name_in_score"}) {
        REQUIRE(classifyStaffDifference(context(std::string("staff[cmper=2]") + suffix, "legacy-mus", DifferenceCategory::Differs))
                == DifferenceClassification::AwaitsDependentRecovery);
        REQUIRE_FALSE(classifyStaffDifference(context(std::string("staff[cmper=1]") + suffix, "legacy-mus", DifferenceCategory::Differs)));
    }
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=2].use_note_font", "legacy-mus", DifferenceCategory::Differs)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=2].note_font.font_size", "unmapped", DifferenceCategory::Differs)));
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=2].note_font.font_size", "legacy-mus", DifferenceCategory::ReaderOnly)));
    REQUIRE(classifyStaffDifference(context("staff[part_id=3,cmper=2].full_name_text_id", "legacy-mus", DifferenceCategory::Differs))
            == DifferenceClassification::AwaitsDependentRecovery);
    REQUIRE_FALSE(classifyStaffDifference(context("staff[cmper=invalid].full_name_text_id", "legacy-mus", DifferenceCategory::Differs)));

    setDeferredRecoveryClassified(false);
    CHECK_FALSE(classifyStaffDifference(context("staff[cmper=2].note_font.font_size", "legacy-mus", DifferenceCategory::Differs)));
    setDeferredRecoveryClassified(true);
}
    ...
    "[coverage]")
{
    ...
    const SourceVersion finale2008{.major = finale_mus_reader::versions::finale2008.major};
    const SourceVersion finale2009{.major = finale_mus_reader::versions::finale2009.major};
    const auto context = [&](const ComparisonLeaves& source, FormatEpoch epoch, const SourceVersion* version, const Value* sourceValue = nullptr,
                             const Value* companionValue = nullptr, std::string_view suffix = ".hide_fretboards") {
        const auto path = suffix == ".hide_chords" ? "staff_style[cmper=5].hide_chords" : "staff_style[cmper=5].hide_fretboards";
        return DifferenceContext{path, DifferenceCategory::Differs, "legacy-mus", sourceValue ? *sourceValue : enabled,
            companionValue ? *companionValue : disabled, source, companionLeaves, epoch, ByteOrder::BigEndian, version, report};
    };

    for (const auto suffix : {".hide_chords", ".hide_fretboards"}) {
        REQUIRE(classify(context(sourceLeaves, FormatEpoch::UncompressedLegacy, &finale2000, nullptr, nullptr, suffix))
                == DifferenceClassification::FinaleUpgradeLoss);
        REQUIRE(classify(context(sourceLeaves, FormatEpoch::DclLegacy, &finale2006, nullptr, nullptr, suffix))
                == DifferenceClassification::FinaleUpgradeLoss);
        REQUIRE(classify(context(sourceLeaves, FormatEpoch::ZlibLegacy, &finale2008, nullptr, nullptr, suffix))
                == DifferenceClassification::FinaleUpgradeLoss);
    }
    ...
    CHECK_FALSE(classify(wrongOrigin));
    CHECK_FALSE(classify(context(sourceLeaves, FormatEpoch::DclLegacy, &finale2006, &disabled, &enabled)));
}
...
TEST_CASE("Legacy StaffStyle Smart Shape values can change when Finale expands them", "[coverage]")
{
    ...
    const SourceVersion finale2000{.major = finale_mus_reader::versions::finale2000.major};
    const SourceVersion finale2009{.major = finale_mus_reader::versions::finale2009.major};
    const auto context = [&](std::string_view path, std::string_view origin, const Value& source, const Value& companion,
                             const SourceVersion* version = nullptr) {
        return DifferenceContext{path, DifferenceCategory::Differs, origin, source, companion, leaves, leaves, FormatEpoch::UncompressedLegacy,
            ByteOrder::BigEndian, version ? version : &finale2000, report};
    };

    REQUIRE(classify(context("staff_style[cmper=2].alt_hide_smart_shapes", "legacy-mus-adjusted", enabled, disabled))
            == DifferenceClassification::FinaleUpgradeLoss);
    CHECK_FALSE(classify(context("staff_style[cmper=2].alt_hide_smart_shapes", "legacy-mus", enabled, disabled)));
    CHECK_FALSE(classify(context("staff_style[cmper=2].alt_hide_smart_shapes", "legacy-mus-adjusted", disabled, enabled)));
    auto finale2009Context = context("staff_style[cmper=2].alt_hide_smart_shapes", "legacy-mus-adjusted", enabled, disabled, &finale2009);
    finale2009Context.epoch = FormatEpoch::ZlibLegacy;
    CHECK_FALSE(classify(finale2009Context));

    const auto key = finale_mus_reader::instanceKey<StaffStyle>(musx::dom::SCORE_PARTID, musx::dom::Cmper{2});
    report.setField(key, "altHideOtherArtics", {ValueOrigin::LegacyMus, 10, 20, 0, std::uint16_t{0x5359}});
    report.setField(key, "altHideOtherSmartShapes", {ValueOrigin::LegacyMus, 10, 20, 0, std::uint16_t{0x5359}});
    REQUIRE(classify(context("staff_style[cmper=2].alt_hide_other_smart_shapes", "legacy-mus", disabled, enabled))
            == DifferenceClassification::FinaleUpgradeLoss);
    REQUIRE(classify(context("staff_style[cmper=2].alt_hide_other_smart_shapes", "legacy-mus", enabled, disabled))
            == DifferenceClassification::FinaleUpgradeLoss);

    report.findField(key, "altHideOtherSmartShapes")->decodedOffset = 22;
    CHECK_FALSE(classify(context("staff_style[cmper=2].alt_hide_other_smart_shapes", "legacy-mus", disabled, enabled)));
    CHECK_FALSE(classify(context("staff_style[cmper=2].alt_hide_other_smart_shapes", "legacy-mus-adjusted", disabled, enabled)));
    CHECK_FALSE(classify(context("staff_style[cmper=2].alt_hide_other_expressions", "legacy-mus", disabled, enabled)));
}
    ...
    "[coverage]")
{
    ...
    ComparisonLeaves source{
        {std::string(objectPath) + ".alt_notation", {Value(static_cast<std::int64_t>(Staff::AlternateNotation::TwoBarRepeat)), "legacy-mus"}},
        {std::string(objectPath) + ".alt_hide_other_artics", {Value(true), "legacy-mus"}}};
    const ComparisonLeaves companion = source;
    ImportReport report(FormatEpoch::UncompressedLegacy);
    const auto key = finale_mus_reader::instanceKey<StaffStyle>(musx::dom::SCORE_PARTID, musx::dom::Cmper{6});
    report.setField(key, "altHideOtherArtics", {ValueOrigin::LegacyMus, 10, 20, 0, std::uint16_t{0x5359}});
    report.setField(key, "altHideOtherSmartShapes", {ValueOrigin::LegacyMus, 10, 20, 0, std::uint16_t{0x5359}});
    ...
    const auto classifyCase = [&](std::string_view suffix, FormatEpoch epoch, const SourceVersion* version, std::string_view origin,
                                  const Value& sourceValue, const Value& companionValue, const ComparisonLeaves& sourceLeaves) {
        const auto path = std::string(objectPath) + std::string(suffix);
        return classify(DifferenceContext{path, DifferenceCategory::Differs, origin, sourceValue, companionValue, sourceLeaves, companion, epoch,
            ByteOrder::BigEndian, version, report});
    };
    const auto classifyUpgrade = [&](std::string_view suffix, FormatEpoch epoch, const SourceVersion* version) {
    ...
    for (const auto suffix : {".alt_hide_artics", ".alt_hide_lyrics"}) {
        CHECK(classifyUpgrade(suffix, FormatEpoch::UncompressedLegacy, &finale2004) == DifferenceClassification::FinaleUpgradeLoss);
        CHECK(classifyUpgrade(suffix, FormatEpoch::ZlibLegacy, &finale2007) == DifferenceClassification::FinaleUpgradeLoss);
    }
    for (const auto suffix : {".hide_chords", ".hide_fretboards"}) {
        CHECK(classifyUpgrade(suffix, FormatEpoch::UncompressedLegacy, &finale2004) == DifferenceClassification::FinaleUpgradeLoss);
    }
    ...
    CHECK_FALSE(classifyCase(".hide_chords", FormatEpoch::UncompressedLegacy, &finale2004, "legacy-mus", visible, hidden, otherItemsVisible));

    auto wrongNotation = source;
    wrongNotation.at(std::string(objectPath) + ".alt_notation").first = Value(static_cast<std::int64_t>(Staff::AlternateNotation::OneBarRepeat));
    CHECK_FALSE(classifyCase(".alt_hide_artics", FormatEpoch::ZlibLegacy, &finale2007, "legacy-mus", visible, hidden, wrongNotation));
    CHECK_FALSE(classifyCase(".alt_hide_artics", FormatEpoch::ZlibLegacy, &finale2007, "legacy-mus-adjusted", visible, hidden, source));
    CHECK_FALSE(classifyCase(".alt_hide_artics", FormatEpoch::ZlibLegacy, &finale2007, "legacy-mus", hidden, visible, source));
    report.findField(key, "altHideOtherSmartShapes")->decodedOffset = 22;
...
TEST_CASE("StaffStyle Unknown UUID upgrades are normalized", "[coverage]")
{
    ...
    const ComparisonLeaves leaves{{objectPath + ".inst_uuid", {Value(std::string(musx::dom::uuid::Unknown)), "legacy-behavior"}},
        {objectPath + ".notation_style", {Value(static_cast<std::int64_t>(Staff::NotationStyle::Percussion)), "legacy-mus"}},
        {objectPath + ".masks.notation_style", {Value(true), "legacy-mus"}}, {objectPath + ".masks.default_clef", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.float_notehead_font", {Value(true), "legacy-mus"}}, {objectPath + ".masks.no_key", {Value(true), "legacy-mus"}},
        {objectPath + ".masks.show_note_colors", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.hide_key_sigs_show_accis", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.staff_type", {Value(true), "legacy-mus"}}, {objectPath + ".masks.transposition", {Value(true), "legacy-mus"}},
        {objectPath + ".masks.full_name", {Value(true), "legacy-mus"}}, {objectPath + ".masks.abrv_name", {Value(true), "legacy-mus"}},
        {objectPath + ".default_clef", {Value(std::int64_t(0)), "legacy-mus"}}, {objectPath + ".use_note_font", {Value(true), "legacy-mus"}},
        {objectPath + ".no_key", {Value(true), "legacy-mus"}}, {objectPath + ".show_note_colors", {Value(false), "legacy-mus"}},
        {objectPath + ".hide_key_sigs_show_accis", {Value(false), "legacy-mus"}}};
    ComparisonLeaves companionLeaves = leaves;
    companionLeaves.at(objectPath + ".inst_uuid").first = Value(std::string(musx::dom::uuid::PercussionGeneral));
    ...
    const auto context = [&](const Value& source, const Value& companion, const ComparisonLeaves* sourceLeaves = nullptr) {
        return DifferenceContext{"staff_style[cmper=1].inst_uuid", DifferenceCategory::Differs, "legacy-behavior", source, companion,
            sourceLeaves ? *sourceLeaves : leaves, companionLeaves, FormatEpoch::DclLegacy, ByteOrder::BigEndian, nullptr, report};
    };

    CHECK(classify(context(unknown, blank)) == DifferenceClassification::FinaleUpgradeNormalization);
    CHECK(classify(context(unknown, blank2)) == DifferenceClassification::FinaleUpgradeNormalization);
    CHECK(classify(context(unknown, percussion)) == DifferenceClassification::FinaleUpgradeNormalization);
    CHECK(classify(context(unknown, flute)) == DifferenceClassification::FinaleUpgradeNormalization);
    CHECK_FALSE(classify(context(blank, unknown)));

    for (const auto suffix :
        {".masks.default_clef", ".masks.float_notehead_font", ".masks.no_key", ".masks.show_note_colors", ".masks.hide_key_sigs_show_accis"}) {
        const auto path = objectPath + suffix;
        const DifferenceContext maskContext{path, DifferenceCategory::Differs, "legacy-mus", leaves.at(path).first, companionLeaves.at(path).first,
            leaves, companionLeaves, FormatEpoch::DclLegacy, ByteOrder::BigEndian, nullptr, report};
    ...
    CHECK(classify(context(unknown, percussion, &unmaskedPercussion)) == DifferenceClassification::FinaleUpgradeNormalization);
    auto standardNotation = leaves;
    standardNotation.at(objectPath + ".notation_style").first = Value(static_cast<std::int64_t>(Staff::NotationStyle::Standard));
    CHECK(classify(context(unknown, percussion, &standardNotation)) == DifferenceClassification::FinaleUpgradeNormalization);
    ...
    const DifferenceContext changedNoKey{noKeyMaskPath, DifferenceCategory::Differs, "legacy-mus", leaves.at(noKeyMaskPath).first,
        companionLeaves.at(noKeyMaskPath).first, leaves, changedActualSetting, FormatEpoch::DclLegacy, ByteOrder::BigEndian, nullptr, report};
...
TEST_CASE("Finale conversion may truncate a StaffStyle name", "[coverage]")
{
    ...
    const auto classifyNames = [&](std::string_view path, std::string_view source, std::string_view companion,
        ...
        return classify(DifferenceContext{
            path, category, origin, sourceValue, companionValue, leaves, leaves, FormatEpoch::ZlibLegacy, ByteOrder::BigEndian, nullptr, report});
    };

    CHECK(classifyNames("staff_style[cmper=5].style_name", "$$$Full Name$$$ Bass Clarinet in Bb\n(actual sou", "$$$Full Name$$$ Bass Cla")
          == DifferenceClassification::FinaleUpgradeLoss);
    CHECK_FALSE(classifyNames("staff_style[cmper=5].style_name", "Long Style Name", "Different Name"));
    CHECK_FALSE(classifyNames("staff_style[cmper=5].style_name", "Long Style Name", ""));
    CHECK_FALSE(classifyNames("staff_style[cmper=5].style_name", "Long Style Name", "Long", DifferenceCategory::ReaderOnly));
    CHECK_FALSE(classifyNames("staff[cmper=5].style_name", "Long Style Name", "Long"));
    CHECK_FALSE(classifyNames("staff_style[cmper=5].style_name", "Long Style Name", "Long", DifferenceCategory::Differs, "legacy-behavior"));
}
...
TEST_CASE("Finale conversion may reinterpret a StaffStyle name through Mac Roman", "[coverage]")
{
    ...
    const auto classifyNames = [&](std::string_view path, std::string_view source, std::string_view companion,
                                   DifferenceCategory category = DifferenceCategory::Differs, std::string_view origin = "legacy-mus",
                                   const SourceVersion* version = nullptr) {
        return classify(DifferenceContext{path, category, origin, Value(std::string(source)), Value(std::string(companion)), leaves, leaves,
            FormatEpoch::ZlibLegacy, ByteOrder::BigEndian, version ? version : &finale2010, report});
    };

    CHECK(classifyNames("staff_style[cmper=14].style_name", "17.  Transposition: Flöte", "17.  Transposition: Flˆte")
          == DifferenceClassification::TextEncodingError);
    CHECK(classifyNames("staff_style[cmper=19].style_name", "15.  Notenköpfe", "15.  Notenkˆpfe") == DifferenceClassification::TextEncodingError);
    ...
    CHECK_FALSE(classifyNames("staff_style[cmper=14].style_name", "Flöte", "Flˆte", DifferenceCategory::ReaderOnly));
    CHECK_FALSE(classifyNames("staff_style[cmper=14].style_name", "Flöte", "Flˆte", DifferenceCategory::Differs, "legacy-behavior"));
    CHECK_FALSE(classifyNames("staff_style[cmper=14].style_name", "Flöte", "Flˆte", DifferenceCategory::Differs, "legacy-mus", &finale2012));
}
...
TEST_CASE("StaffStyle names compare without surrounding whitespace", "[coverage]")
{
    ...
    const auto snapshot = [](std::string name) {
        return SurveySnapshot{{"staff_style", Value::Array{Value::Object{{"cmper", 1}, {"style_name", std::move(name)}}}}};
    };
    const auto sourceDocument = emptyStaffDocument();
    const auto companionDocument = emptyStaffDocument();
    ImportReport report(FormatEpoch::UncompressedLegacy);
    const auto comparison = compareSnapshots(snapshot("  Normal Notation"), snapshot("Normal Notation \t"), sourceDocument, companionDocument,
...
TEST_CASE("StaffStyle comparison ignores tablature fields outside Tablature notation", "[coverage]")
{
    using namespace finale_mus_reader::coverage;
    const auto snapshot = [](Staff::NotationStyle notation, std::int64_t offset) {
        return SurveySnapshot{
            {"staff_style", Value::Array{Value::Object{{"cmper", Value(1)}, {"notation_style", Value(static_cast<std::int64_t>(notation))},
                                {"vert_tab_num_off", Value(offset)}}}}};
    };
    const auto unexpected = [&](Staff::NotationStyle notation) {
        ImportReport report(FormatEpoch::ZlibLegacy);
        const auto comparison = compareSnapshots(snapshot(notation, 0), snapshot(notation, -1024), emptyStaffDocument(), emptyStaffDocument(),
...
TEST_CASE("Finale 2009 beta legacy StaffStyle layouts are not compared", "[coverage]")
{
    ...
    const auto comparisonWasSkipped = [](const SourceVersion& version, bool aggregateLayout) {
        ...
        ImportReport report(FormatEpoch::ZlibLegacy);
        const auto instance = finale_mus_reader::instanceKey<StaffStyle>(musx::dom::SCORE_PARTID, musx::dom::Cmper{1});
        report.setField(instance, "altHideOtherArtics", {ValueOrigin::LegacyMus, 10, 20, 0, std::uint16_t{0x00e8}});
        report.setField(instance, "altHideOtherSmartShapes",
            {ValueOrigin::LegacyMus, 10, aggregateLayout ? std::size_t{20} : std::size_t{22}, 0, std::uint16_t{0x00e8}});
        ComparisonPreparationContext context{source, companion, transformations, FormatEpoch::ZlibLegacy, &version, &report};
        runComparisonPreparers(context);
        return !source.contains("staff_style") && !companion.contains("staff_style");
    };

    const SourceVersion finale2009Beta{.major = finale_mus_reader::versions::finale2009.major, .devStatus = 2};
    const SourceVersion finale2009Release{.major = finale_mus_reader::versions::finale2009.major, .devStatus = 4};
    const SourceVersion finale2010Beta{.major = finale_mus_reader::versions::finale2010.major, .devStatus = 2};
...
TEST_CASE("StaffStyle menu metadata does not affect semantic coverage", "[coverage]")
{
    ...
    const ComparisonLeaves leaves;
    ImportReport report(FormatEpoch::ZlibLegacy);
    const auto classifyBoolean = [&](std::string_view path, bool source, bool companion, DifferenceCategory category = DifferenceCategory::Differs) {
        return classify(DifferenceContext{path, category, "legacy-mus", Value(source), Value(companion), leaves, leaves, FormatEpoch::ZlibLegacy,
            ByteOrder::BigEndian, nullptr, report});
    };

    for (const auto suffix : {".copyable", ".add_to_menu"}) {
        CHECK(classifyBoolean(std::string("staff_style[cmper=5]") + suffix, true, false) == DifferenceClassification::DifferentDefaults);
        CHECK(classifyBoolean(std::string("staff_style[cmper=5]") + suffix, false, true) == DifferenceClassification::DifferentDefaults);
    }
    CHECK_FALSE(classifyBoolean("staff_style[cmper=5].copyable", true, false, DifferenceCategory::ReaderOnly));
...
TEST_CASE("Pre-Finale 2012 instrument StaffStyles acquire every required mask", "[coverage]")
{
    ...
    REQUIRE(classify);
    const auto objectPath = std::string("staff_style[cmper=1]");
    ComparisonLeaves source{{objectPath + ".inst_uuid", {Value(std::string(musx::dom::uuid::Unknown)), "legacy-behavior"}},
        {objectPath + ".notation_style", {Value(static_cast<std::int64_t>(Staff::NotationStyle::Standard)), "legacy-mus"}},
        {objectPath + ".default_clef", {Value(std::int64_t(3)), "legacy-mus"}}, {objectPath + ".show_note_colors", {Value(false), "legacy-mus"}},
        {objectPath + ".hide_key_sigs_show_accis", {Value(false), "legacy-mus"}},
        {objectPath + ".transposition.present", {Value(true), "legacy-mus"}},
        {objectPath + ".transposition.chromatic.alteration", {Value(std::int64_t(0)), "legacy-mus"}},
        {objectPath + ".transposition.chromatic.diatonic", {Value(std::int64_t(3)), "legacy-mus"}},
        {objectPath + ".style_name", {Value("Source"), "legacy-mus"}}, {objectPath + ".masks.notation_style", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.default_clef", {Value(true), "legacy-mus"}}, {objectPath + ".masks.show_note_colors", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.hide_key_sigs_show_accis", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.staff_type", {Value(true), "legacy-mus"}}, {objectPath + ".masks.transposition", {Value(true), "legacy-mus"}},
        {objectPath + ".masks.full_name", {Value(true), "legacy-mus"}}, {objectPath + ".masks.abrv_name", {Value(true), "legacy-mus"}}};
    ...
    const SourceVersion finale2006{.major = finale_mus_reader::versions::finale2006.major};
    const SourceVersion finale2012{.major = finale_mus_reader::versions::finale2012.major};
    const auto context = [&](std::string_view path, const ComparisonLeaves& sourceLeaves, const ComparisonLeaves& companionLeaves,
                             std::string_view origin = "legacy-mus") {
        return DifferenceContext{path, DifferenceCategory::Differs, origin, sourceLeaves.at(std::string(path)).first,
            companionLeaves.at(std::string(path)).first, sourceLeaves, companionLeaves, FormatEpoch::DclLegacy, ByteOrder::BigEndian, &finale2006,
            report};
    };

    for (const auto suffix : {".masks.notation_style", ".masks.show_note_colors", ".masks.hide_key_sigs_show_accis"}) {
        const auto path = objectPath + suffix;
        CHECK(classify(context(path, source, companion)) == DifferenceClassification::FinaleUpgradeNormalization);
    }
    const auto uuidPath = objectPath + ".inst_uuid";
    CHECK(classify(context(uuidPath, source, companion, "legacy-behavior")) == DifferenceClassification::FinaleUpgradeNormalization);

    auto changedValue = companion;
    changedValue.at(objectPath + ".show_note_colors").first = Value(true);
    CHECK(classify(context(objectPath + ".masks.show_note_colors", source, changedValue)) == DifferenceClassification::FinaleUpgradeNormalization);
    CHECK(classify(context(objectPath + ".show_note_colors", source, changedValue)) == DifferenceClassification::FinaleUpgradeNormalization);

    auto changedTransposition = companion;
    changedTransposition.at(objectPath + ".transposition.present").first = Value(false);
    changedTransposition.at(objectPath + ".transposition.chromatic.alteration").first = Value(std::int64_t(-1));
    changedTransposition.at(objectPath + ".transposition.chromatic.diatonic").first = Value(std::int64_t(4));
    for (const auto suffix : {".transposition.present", ".transposition.chromatic.alteration", ".transposition.chromatic.diatonic"}) {
        CHECK(classify(context(objectPath + suffix, source, changedTransposition)) == DifferenceClassification::FinaleUpgradeNormalization);
    }

    auto dormantTransposition = source;
    dormantTransposition.at(objectPath + ".masks.transposition").first = Value(false);
    CHECK(classify(context(objectPath + ".transposition.present", dormantTransposition, changedTransposition))
    ...
    CHECK(classify(context(objectPath + ".style_name", source, changedName)) == DifferenceClassification::FinaleUpgradeLoss);

    auto incompleteCompanion = companion;
    incompleteCompanion.at(objectPath + ".masks.hide_key_sigs_show_accis").first = Value(false);
    CHECK(
        classify(context(objectPath + ".masks.notation_style", source, incompleteCompanion)) == DifferenceClassification::FinaleUpgradeNormalization);
    CHECK(classify(context(uuidPath, source, incompleteCompanion, "legacy-behavior")) == DifferenceClassification::FinaleUpgradeNormalization);

    auto sourceWithoutInstrumentMasks = source;
    sourceWithoutInstrumentMasks.at(objectPath + ".masks.default_clef").first = Value(false);
    CHECK(classify(context(objectPath + ".masks.notation_style", sourceWithoutInstrumentMasks, companion))
          == DifferenceClassification::FinaleUpgradeNormalization);

    ComparisonLeaves pluginSource{{objectPath + ".masks.notation_style", {Value(true), "legacy-mus"}},
        {objectPath + ".masks.default_clef", {Value(false), "legacy-mus"}}, {objectPath + ".masks.show_note_colors", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.hide_key_sigs_show_accis", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.staff_type", {Value(false), "legacy-mus"}}, {objectPath + ".masks.transposition", {Value(false), "legacy-mus"}},
        {objectPath + ".masks.full_name", {Value(false), "legacy-mus"}}, {objectPath + ".masks.abrv_name", {Value(false), "legacy-mus"}},
        {objectPath + ".default_clef", {Value(std::int64_t(0)), "legacy-mus"}}, {objectPath + ".show_note_colors", {Value(false), "legacy-mus"}},
    ...
    for (const auto suffix :
        {".masks.default_clef", ".masks.show_note_colors", ".masks.staff_type", ".masks.transposition", ".masks.full_name", ".masks.abrv_name"}) {
    ...
    for (const auto suffix : {".masks.default_clef", ".masks.show_note_colors", ".masks.staff_type", ".masks.transposition", ".masks.full_name",
             ".masks.abrv_name", ".default_clef", ".full_name_text_id", ".abbrv_name_text_id"}) {
        CHECK(classify(context(objectPath + suffix, pluginSource, pluginCompanion)) == DifferenceClassification::FinaleUpgradeNormalization);
    }

    auto incompletePluginCompanion = pluginCompanion;
    incompletePluginCompanion.at(objectPath + ".masks.abrv_name").first = Value(false);
    CHECK_FALSE(classify(context(objectPath + ".masks.default_clef", pluginSource, incompletePluginCompanion)));
    ...
    removedSource.emplace(objectPath + ".staff_lines", std::pair{Value(std::int64_t(0)), "legacy-mus"});
    removedSource.emplace(objectPath + ".transposed_clef", std::pair{Value(std::int64_t(4)), "legacy-mus"});
    removedSource.emplace(objectPath + ".transposition.present", std::pair{Value(true), "legacy-mus"});
    removedSource.emplace(objectPath + ".full_name_text_id", std::pair{Value(std::int64_t(311)), "legacy-mus"});
    removedSource.emplace(objectPath + ".abbrv_name_text_id", std::pair{Value(std::int64_t(312)), "legacy-mus"});
    auto removedCompanion = removedSource;
    for (const auto suffix : {".masks.notation_style", ".masks.default_clef", ".masks.show_note_colors", ".masks.hide_key_sigs_show_accis",
             ".masks.staff_type", ".masks.transposition", ".masks.full_name", ".masks.abrv_name"}) {
    ...
    for (const auto suffix : {".masks.default_clef", ".masks.staff_type", ".masks.transposition", ".masks.full_name", ".masks.abrv_name",
             ".default_clef", ".staff_lines", ".transposed_clef", ".transposition.present", ".full_name_text_id", ".abbrv_name_text_id"}) {
        CHECK(classify(context(objectPath + suffix, removedSource, removedCompanion)) == DifferenceClassification::FinaleUpgradeNormalization);
    }

    auto removedPost2012 = context(objectPath + ".full_name_text_id", removedSource, removedCompanion);
...
TEST_CASE("Pre-Finale 2012 mixed StaffStyles split into companion styles", "[coverage]")
{
    ...
    const ComparisonLeaves source{{std::string(retained) + ".style_name", {Value("Mixed"), "legacy-mus"}},
        ...
        {std::string(retained) + ".notation_style", {Value(static_cast<std::int64_t>(Staff::NotationStyle::Percussion)), "legacy-mus"}},
        ...
        {std::string(retained) + ".use_note_font", {Value(true), "legacy-mus"}}, {std::string(retained) + ".no_key", {Value(true), "legacy-mus"}},
        {std::string(retained) + ".bot_barline_offset", {Value(-24), "legacy-mus"}},
        {std::string(retained) + ".dw_rest_offset", {Value(-5), "legacy-mus"}}, {std::string(retained) + ".staff_lines", {Value(0), "legacy-mus"}},
        {std::string(retained) + ".top_barline_offset", {Value(24), "legacy-mus"}},
        {std::string(retained) + ".w_rest_offset", {Value(-6), "legacy-mus"}}, {std::string(retained) + ".line_space", {Value(24), "legacy-mus"}}};
    ComparisonLeaves companion{{std::string(retained) + ".style_name", {Value("Mixed "), {}}},
        {std::string(retained) + ".masks.flat_beams", {Value(true), {}}}, {std::string(retained) + ".masks.notation_style", {Value(false), {}}},
        {std::string(retained) + ".masks.default_clef", {Value(false), {}}}, {std::string(retained) + ".masks.show_note_colors", {Value(false), {}}},
        {std::string(retained) + ".masks.hide_key_sigs_show_accis", {Value(false), {}}},
        {std::string(retained) + ".masks.staff_type", {Value(false), {}}}, {std::string(retained) + ".masks.transposition", {Value(false), {}}},
        {std::string(retained) + ".masks.float_notehead_font", {Value(false), {}}}, {std::string(retained) + ".masks.no_key", {Value(false), {}}},
        {std::string(retained) + ".notation_style", {Value(static_cast<std::int64_t>(Staff::NotationStyle::Standard)), {}}},
        {std::string(retained) + ".default_clef", {Value(std::int64_t(0)), {}}}, {std::string(retained) + ".show_note_colors", {Value(false), {}}},
        {std::string(retained) + ".hide_key_sigs_show_accis", {Value(false), {}}},
        {std::string(retained) + ".transposition.present", {Value(false), {}}},
        {std::string(retained) + ".custom_staff[0]", {Value(std::int64_t(11)), {}}}, {std::string(retained) + ".use_note_font", {Value(true), {}}},
        {std::string(retained) + ".no_key", {Value(true), {}}}, {std::string(retained) + ".bot_barline_offset", {Value(0), {}}},
        {std::string(retained) + ".dw_rest_offset", {Value(-4), {}}}, {std::string(retained) + ".staff_lines", {Value(5), {}}},
        {std::string(retained) + ".top_barline_offset", {Value(0), {}}}, {std::string(retained) + ".w_rest_offset", {Value(-4), {}}},
        {std::string(retained) + ".line_space", {Value(25), {}}}, {std::string(split) + ".style_name", {Value("Mixed "), {}}},
        {std::string(split) + ".masks.notation_style", {Value(true), {}}}, {std::string(split) + ".masks.default_clef", {Value(true), {}}},
        {std::string(split) + ".masks.show_note_colors", {Value(true), {}}},
        {std::string(split) + ".masks.hide_key_sigs_show_accis", {Value(true), {}}}, {std::string(split) + ".masks.staff_type", {Value(true), {}}},
        {std::string(split) + ".masks.transposition", {Value(true), {}}}, {std::string(split) + ".masks.full_name", {Value(true), {}}},
        {std::string(split) + ".masks.abrv_name", {Value(true), {}}},
        {std::string(split) + ".notation_style", {Value(static_cast<std::int64_t>(Staff::NotationStyle::Percussion)), {}}},
        {std::string(split) + ".default_clef", {Value(std::int64_t(12)), {}}}, {std::string(split) + ".show_note_colors", {Value(false), {}}},
        {std::string(split) + ".hide_key_sigs_show_accis", {Value(false), {}}}, {std::string(split) + ".transposition.present", {Value(true), {}}},
        {std::string(split) + ".custom_staff[0]", {Value(std::int64_t(13)), {}}}, {std::string(split) + ".bot_barline_offset", {Value(-24), {}}},
        {std::string(split) + ".dw_rest_offset", {Value(-5), {}}}, {std::string(split) + ".staff_lines", {Value(0), {}}},
        {std::string(split) + ".top_barline_offset", {Value(24), {}}}, {std::string(split) + ".w_rest_offset", {Value(-6), {}}},
    ...
    const SourceVersion finale2012{.major = finale_mus_reader::versions::finale2012.major};
    const auto contextFrom = [&](std::string_view path, const ComparisonLeaves& sourceLeaves, const ComparisonLeaves& companionLeaves,
                                 const SourceVersion* version) {
        return DifferenceContext{path, DifferenceCategory::Differs, "legacy-mus", sourceLeaves.at(std::string(path)).first,
            companionLeaves.at(std::string(path)).first, sourceLeaves, companionLeaves, FormatEpoch::ZlibLegacy, ByteOrder::BigEndian, version,
            report};
    };
    const auto context = [&](std::string_view path, const ComparisonLeaves& companionLeaves, const SourceVersion* version) {
        return contextFrom(path, source, companionLeaves, version);
    };

    for (const auto suffix : {".masks.notation_style", ".masks.default_clef", ".masks.staff_type", ".masks.transposition", ".bot_barline_offset",
             ".dw_rest_offset", ".staff_lines", ".top_barline_offset", ".transposition.present", ".custom_staff[0]", ".w_rest_offset"}) {
        CHECK(classify(context(std::string(retained) + suffix, companion, &finale2011)) == DifferenceClassification::FinaleUpgradeNormalization);
    }
    ...
    sourceWithoutFormerGate.at(std::string(retained) + ".masks.notation_style").first = Value(false);
    sourceWithoutFormerGate.at(std::string(retained) + ".masks.staff_type").first = Value(false);
    CHECK(classify(contextFrom(std::string(retained) + ".transposition.present", sourceWithoutFormerGate, companion, &finale2011))
          == DifferenceClassification::FinaleUpgradeNormalization);

    auto mismatchedFork = companion;
    mismatchedFork.at(std::string(split) + ".custom_staff[0]").first = Value(std::int64_t(12));
    CHECK_FALSE(classify(context(std::string(retained) + ".custom_staff[0]", mismatchedFork, &finale2011)));

    auto noRetainedStyleSource = source;
    noRetainedStyleSource.at(std::string(retained) + ".masks.flat_beams").first = Value(false);
    CHECK_FALSE(classify(contextFrom(std::string(retained) + ".staff_lines", noRetainedStyleSource, companion, &finale2011)));

    auto incompleteSplit = companion;
    incompleteSplit.erase(std::string(split) + ".masks.staff_type");
    CHECK_FALSE(classify(context(std::string(retained) + ".staff_lines", incompleteSplit, &finale2011)));

    const Value absent;
    const DifferenceContext synthesizedNoteFont{"staff_style[cmper=7].use_note_font", DifferenceCategory::CompanionOnly, {}, absent,
        companion.at("staff_style[cmper=7].use_note_font").first, source, companion, FormatEpoch::ZlibLegacy, ByteOrder::BigEndian, &finale2011,
        report};
    ...
    "[coverage]")
{
    ...
    REQUIRE(classify);
    ImportReport report(FormatEpoch::UncompressedLegacy);
    const auto classifyValue = [&](std::string_view valueSuffix, std::string_view maskSuffix, bool sourceMask = false, bool companionMask = false,
                                   DifferenceCategory category = DifferenceCategory::Differs, std::string_view classPrefix = "staff_style[cmper=1]") {
        ...
        return classify(DifferenceContext{valuePath, category, "finale27-default", sourceValue, companionValue, source, companion,
            FormatEpoch::UncompressedLegacy, ByteOrder::BigEndian, nullptr, report});
    };
    ...
    for (const auto& [valueSuffix, maskSuffix] : governedValues) {
        CHECK(classifyValue(valueSuffix, maskSuffix) == DifferenceClassification::DifferentDefaults);
    }
    ...
    CHECK_FALSE(classifyValue(".line_space", ".masks.staff_type", false, false, DifferenceCategory::ReaderOnly));
    CHECK_FALSE(classifyValue(".style_name", ".masks.staff_type"));
    CHECK_FALSE(classifyValue(".line_space", ".masks.staff_type", false, false, DifferenceCategory::Differs, "staff[cmper=1]"));
    ...
    ComparisonLeaves source{{useFontPath, {Value(false), "legacy-mus"}}, {maskPath, {Value(true), "legacy-mus"}}};
    const ComparisonLeaves companion{{useFontPath, {Value(false), {}}}, {maskPath, {Value(true), {}}}};
    const auto disabledFontContext = [&] {
        return DifferenceContext{sizePath, DifferenceCategory::Differs, "legacy-mus", storedZero, materializedSize, source, companion,
            FormatEpoch::UncompressedLegacy, ByteOrder::BigEndian, nullptr, report};
    };
