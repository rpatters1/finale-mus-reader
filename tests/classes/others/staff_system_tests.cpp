...
void addStaffSystemTestMeasures(const musx::dom::DocumentPtr& document,
                                int lastMeasure)
{
    for (int measureId = 1; measureId <= lastMeasure; ++measureId) {
        auto measure =
            std::make_shared<Measure>(document, musx::dom::SCORE_PARTID,
                                      musx::dom::EnigmaBase::ShareMode::None,
                                      musx::dom::Cmper(measureId));
        document->getOthers()->add(Measure::XmlNodeName, std::move(measure));
    }
}

std::vector<std::int16_t> staffSystemWords(std::int16_t flags = 0x001f,
                                           std::int16_t staffHeight = 1536,
                                           bool extended = true)
{
    ...
    if (extended) result.insert(result.end(), {24, 36});
    return result;
}

void appendExpandedStaffSystemRows(std::vector<SyntheticRow>& rows,
                                   std::uint16_t systemId,
                                   std::int16_t startMeas,
                                   std::int16_t storedEndMeas)
{
    auto words = staffSystemWords(0, 1536, false);
    words[4] = startMeas;
    words[6] = storedEndMeas;
    for (std::size_t at = 0; at < words.size(); at += 6) {
        rows.push_back({systemId,
                        "SS",
                        {words[at], words[at + 1], words[at + 2], words[at + 3],
                         words[at + 4], words[at + 5]}});
    }
}

void checkStaffSystem(const musx::dom::DocumentPtr& document,
                      const ImportReport& report, int expectedTop,
                      int expectedDistance, ValueOrigin expectedTopOrigin,
                      int expectedTopRaw,
                      musx::dom::Cmper systemId = musx::dom::Cmper(1),
                      bool extended = true)
{
    const auto system = document->getOthers()->get<StaffSystem>(
        musx::dom::SCORE_PARTID, systemId);
    REQUIRE(system);
    ...
    for (const auto* member :
         {"startMeas", "horzPercent", "ssysPercent", "staffHeight", "left",
          "right", "bottom", "noNames", "placeEndSpaceBeforeBarline",
          "scaleVert", "holdMargins", "distanceToPrev"}) {
        const auto* source = report.findField<StaffSystem>(
            member, musx::dom::SCORE_PARTID, systemId);
        REQUIRE(source);
        CHECK(source->origin == ValueOrigin::LegacyMus);
    }
    const auto* endSource = report.findField<StaffSystem>(
        "endMeas", musx::dom::SCORE_PARTID, systemId);
    REQUIRE(endSource);
    CHECK(endSource->origin == ValueOrigin::LegacyMusAdjusted);
    CHECK(endSource->rawValue == 9);
    const auto* staffScalingSource = report.findField<StaffSystem>(
        "hasStaffScaling", musx::dom::SCORE_PARTID, systemId);
    REQUIRE(staffScalingSource);
    CHECK(staffScalingSource->origin == ValueOrigin::Unmapped);
    const auto* topSource =
        report.findField<StaffSystem>("top", musx::dom::SCORE_PARTID, systemId);
    REQUIRE(topSource);
    CHECK(topSource->origin == expectedTopOrigin);
    for (const auto* member :
         {"extraStartSystemSpace", "extraEndSystemSpace"}) {
        const auto* source = report.findField<StaffSystem>(
            member, musx::dom::SCORE_PARTID, systemId);
        REQUIRE(source);
        CHECK(source->origin == (extended ? ValueOrigin::LegacyMus
                                          : ValueOrigin::LegacyBehavior));
    }
    CHECK(report.findField<StaffSystem>("horzPercent", 0, systemId)->rawValue ==
          12050);
    CHECK(report.findField<StaffSystem>("staffHeight", 0, systemId)->rawValue ==
          1536);
    CHECK(report.findField<StaffSystem>("top", 0, systemId)->rawValue ==
          expectedTopRaw);
    CHECK(report.findField<StaffSystem>("distanceToPrev", 0, systemId)
              ->rawValue == expectedDistance);
}

TEST_CASE("Fixed-row staff systems recover the base and Finale 2005 layouts",
          "[class][staff-system]")
{
    for (const auto epoch :
        ...
        for (const auto byteOrder :
            ...
            for (std::size_t at = 0; at < 18; at += 6) {
                rows.push_back({1,
                                ...
                                {words[at], words[at + 1], words[at + 2],
                                 words[at + 3], words[at + 4], words[at + 5]}});
            }
            ...
            const auto report = importStaffSystems(
                makeContainer(rows, epoch, byteOrder), profile, document);
            const int expectedTop =
                epoch == FormatEpoch::UncompressedLegacy ? 0 : -12;
            const int expectedDistance = -490;
            const auto expectedTopOrigin =
                epoch == FormatEpoch::UncompressedLegacy
                    ? ValueOrigin::Unmapped
                    : ValueOrigin::LegacyMus;
            const int expectedTopRaw =
                epoch == FormatEpoch::UncompressedLegacy ? 0 : -12;
            checkStaffSystem(document, report, expectedTop, expectedDistance,
                             expectedTopOrigin, expectedTopRaw);
        }
    }
    ...
    checkStaffSystem(document, importStaffSystems(parsed, profile, document), 0,
                     -490, ValueOrigin::Unmapped, 0, musx::dom::Cmper(1), false);
}

TEST_CASE(
    "Uncompressed staff-system distance uses the first word after system one",
    "[class][staff-system]")
{
    const auto words = staffSystemWords(0x001f, 1536, false);
    const auto parsed = makeContainer(
    ...
    profile.byteOrder = ByteOrder::BigEndian;
    checkStaffSystem(document, importStaffSystems(parsed, profile, document), 0,
                     -490, ValueOrigin::Unmapped, 0, musx::dom::Cmper(1),
                     false);
}

TEST_CASE(
    "Zlib staff systems recover both byte orders and ignore retired flag bits",
    "[class][staff-system]")
{
    for (const auto byteOrder :
         {ByteOrder::BigEndian, ByteOrder::LittleEndian}) {
        ...
        checkStaffSystem(
            document,
            importStaffSystems(
                makeClassContainer(0x00df, staffSystemWords(), byteOrder, 1),
                profile, document),
            -12, -490, ValueOrigin::LegacyMus, -12);
        ...
        const auto ignored =
            ignoredDocument->getOthers()->get<StaffSystem>(0, 1);
        REQUIRE(ignored);
...
TEST_CASE("A zero legacy staff height takes the standard staff-height behavior",
          "[class][staff-system]")
{
    const auto document = emptyStaffSystemDocument();
    addStaffSystemTestMeasures(document, 8);
    ...
    const auto report =
        importStaffSystems(makeClassContainer(0x00df, staffSystemWords(0, 0),
...
TEST_CASE(
    "Incomplete staff-system layouts are rejected without partial objects",
    "[class][staff-system]")
{
    for (const auto size : {11, 13}) {
        ...
        profile.byteOrder = ByteOrder::LittleEndian;
        const auto report = importStaffSystems(
            makeClassContainer(0x00df, std::vector<std::int16_t>(size),
                               ByteOrder::LittleEndian, 7),
            profile, document);
...
TEST_CASE(
    "Compact staff systems recover independently of their container epoch",
    "[class][staff-system]")
{
    for (const auto epoch :
        ...
        profile.byteOrder = ByteOrder::BigEndian;
        const auto report = importStaffSystems(
            makeContainer({{1, "SS", {-257, 389, -247, -299, 1, 0}},
                           {2, "SS", {-481, 517, -499, -521, 3, 128}}},
                          epoch),
            profile, document);
        ...
        const auto fieldOrigin = [&](const char* member,
                                     musx::dom::Cmper systemId) {
            const auto* field =
                report.findField<StaffSystem>(member, 0, systemId);
            REQUIRE(field);
            return field->origin;
        };
...
TEST_CASE("Compact staff systems stop at the end of their valid sequence",
          "[class][staff-system]")
{
    for (const auto invalidStart : {0, 3, 7}) {
        ...
        const auto report = importStaffSystems(
            makeContainer(
                {{1, "SS", {0, 0, 0, -200, 1, 0}},
                 {2, "SS", {0, 0, 0, -200, 3, 0}},
                 {3, "SS", {0, 0, 0, -200, std::int16_t(invalidStart), 0}},
                 {4, "SS", {0, 0, 0, -200, 5, 0}}},
    ...
    profile.byteOrder = ByteOrder::BigEndian;
    const auto report =
        importStaffSystems(makeContainer({{1, "SS", {0, 0, 0, -200, 1, 0}},
                                          {3, "SS", {0, 0, 0, -200, 3, 0}}},
                                         FormatEpoch::UncompressedLegacy),
                           profile, document);
...
TEST_CASE("Expanded staff systems rebuild a valid system grid",
          "[class][staff-system]")
{
    for (const auto invalidStart : {0, 3, 7}) {
        std::vector<SyntheticRow> rows;
        ...
        profile.byteOrder = ByteOrder::BigEndian;
        const auto report = importStaffSystems(
            makeContainer(rows, FormatEpoch::UncompressedLegacy), profile,
            document);
        const auto first = document->getOthers()->get<StaffSystem>(0, 1);
        const auto second = document->getOthers()->get<StaffSystem>(0, 2);
        REQUIRE(first);
    ...
    profile.byteOrder = ByteOrder::BigEndian;
    const auto report =
        importStaffSystems(makeContainer(rows, FormatEpoch::UncompressedLegacy),
                           profile, document);
    REQUIRE(document->getOthers()->get<StaffSystem>(0, 1));
    CHECK_FALSE(document->getOthers()->get<StaffSystem>(0, 3));
    CHECK(reportedFieldCount(report) == staffSystemFieldCount);
...
TEST_CASE("Coda system scaling rows override the base system options",
          "[class][staff-system]")
{
    const auto document = emptyStaffSystemDocument();
    ...
    profile.byteOrder = ByteOrder::BigEndian;
    const auto report = importStaffSystems(
        makeContainer({{1, "SS", {0, 0, 0, -200, 1, 0x0880}},
                       {2, "SS", {-200, 0, 0, -200, 2, 0x0880}},
                       {3, "SS", {-200, 0, 0, -200, 3, 0x0880}},
                       {1, "SP", {83, 83, 0, 0, 0, 0}},
                       {2, "SP", {85, 85, 0, 0, 0, 0x4000}},
                       {3, "SP", {87, 87, 0, 0, 0, 0x2000}}},
    ...
    for (const auto systemId :
         {musx::dom::Cmper(1), musx::dom::Cmper(2), musx::dom::Cmper(3)}) {
...
TEST_CASE("StaffSystem survey emits the complete persisted field manifest",
          "[class][staff-system][coverage]")
{
    const auto document = emptyStaffSystemDocument();
    auto profile = SourceProfile(FormatEpoch::ZlibLegacy);
    profile.byteOrder = ByteOrder::LittleEndian;
    auto report =
        importStaffSystems(makeClassContainer(0x00df, staffSystemWords(),
                                              ByteOrder::LittleEndian, 1),
                           profile, document);
    const auto surveyed =
        finale_mus_reader::coverage::runAllSurveyors({document, report});
    ...
    const std::set<std::string> expected{"cmper",
                                         "part_id",
                                         "share_mode",
                                         "origin",
                                         "start_meas",
                                         ...
                                         "no_names",
                                         "has_staff_scaling",
                                         "place_end_space_before_barline",
                                         "scale_vert",
                                         "hold_margins",
                                         "distance_to_prev",
                                         "extra_start_system_space",
                                         "extra_end_system_space",
                                         "origin_startMeas",
                                         "origin_endMeas",
                                         "origin_horzPercent",
                                         "origin_ssysPercent",
                                         ...
                                         "origin_bottom",
                                         "origin_noNames",
                                         "origin_hasStaffScaling",
                                         "origin_placeEndSpaceBeforeBarline",
                                         "origin_scaleVert",
                                         "origin_holdMargins",
                                         "origin_distanceToPrev",
                                         "origin_extraStartSystemSpace",
                                         "origin_extraEndSystemSpace"};
...
TEST_CASE("StaffSystem comparison recognizes Finale layout recalculation",
          "[coverage]")
{
    ...
    REQUIRE(classify);
    const auto classifyValues =
        [&](std::string_view member, Value sourceValue, Value companionValue,
            ...
            return classify(DifferenceContext{
                path, category, origin, sourceValue, companionValue, source,
                companion, FormatEpoch::UncompressedLegacy,
                ByteOrder::BigEndian, nullptr, report});
        };
    const auto classifyLayout =
        [&](std::string_view member,
            std::initializer_list<std::int64_t> sourceEnds,
            std::initializer_list<std::int64_t> companionEnds,
            musx::dom::Cmper sourceSystem, musx::dom::Cmper lastMeasure,
            DifferenceCategory category = DifferenceCategory::Differs,
            std::int64_t sourceFirstStart = 1,
            std::int64_t companionFirstStart = 1) {
            source.clear();
            companion.clear();
            sourceDocument.clear();
            const auto addSystems = [](ComparisonLeaves& leaves,
                                       auto endMeasures,
                                       std::int64_t firstStart,
                                       std::string_view origin) {
                musx::dom::Cmper systemId = musx::dom::Cmper(1);
                auto startMeas = firstStart;
                for (const auto endMeas : endMeasures) {
                    const auto prefix = "staff_systems[cmper=" +
                                        std::to_string(systemId++) + "].";
                    leaves.emplace(prefix + "start_meas",
                                   std::pair{Value(startMeas),
                                             std::string(origin)});
                    leaves.emplace(prefix + "end_meas",
                                   std::pair{Value(endMeas),
                                             std::string(origin)});
                    startMeas = endMeas;
                }
            };
            addSystems(source, sourceEnds, sourceFirstStart,
                       "legacy-mus-adjusted");
            addSystems(companion, companionEnds, companionFirstStart,
                       "enigma-xml");
            const auto path =
                "staff_systems[cmper=" + std::to_string(sourceSystem) +
                "]." + std::string(member);
            sourceDocument.emplace(
                "measures[cmper=" + std::to_string(lastMeasure) + "].width",
                std::pair{Value(0), std::string("legacy-mus")});
            return classify(DifferenceContext{
                path, category, source.at(path).second, source.at(path).first,
                companion.at(path).first, source, companion,
                FormatEpoch::UncompressedLegacy, ByteOrder::BigEndian, nullptr,
                report, {}, {}, &sourceDocument});
        };

    CHECK(classifyValues("horz_percent", Value(0.0), Value(331.5)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK_FALSE(classifyValues("start_meas", Value(28), Value(31)));
    CHECK(classifyValues("horz_percent", Value(120.5), Value(0.0)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK(classifyLayout("end_meas", {3}, {2, 3}, musx::dom::Cmper(1),
                         musx::dom::Cmper(2)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK(classifyLayout("end_meas", {3}, {4}, musx::dom::Cmper(1),
                         musx::dom::Cmper(2)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK(classifyLayout("end_meas", {2, 4}, {3, 4},
                         musx::dom::Cmper(1), musx::dom::Cmper(3)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK(classifyLayout("start_meas", {4, 8, 12, 16, 20, 22},
                         {2, 6, 10, 14, 18, 22}, musx::dom::Cmper(2),
                         musx::dom::Cmper(21)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK(classifyLayout("end_meas", {3}, {2}, musx::dom::Cmper(1),
                         musx::dom::Cmper(2)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK(classifyLayout("end_meas", {3}, {2}, musx::dom::Cmper(1),
                         musx::dom::Cmper(2), DifferenceCategory::Differs, 1,
                         0) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK_FALSE(classifyLayout(
        "end_meas", {3}, {2}, musx::dom::Cmper(1), musx::dom::Cmper(2),
        DifferenceCategory::Differs, 0, 1));
    CHECK(classifyLayout("end_meas", {3}, {9, 2}, musx::dom::Cmper(1),
                         musx::dom::Cmper(2)) ==
          DifferenceClassification::FinaleLayoutRecalculation);
    CHECK_FALSE(classifyLayout("end_meas", {2, 4}, {3, 2},
                               musx::dom::Cmper(1), musx::dom::Cmper(3)));
    CHECK_FALSE(classifyLayout("end_meas", {4}, {5},
                               musx::dom::Cmper(1), musx::dom::Cmper(2)));
    CHECK_FALSE(classifyLayout(
        "start_meas", {4, 8}, {2, 8}, musx::dom::Cmper(2),
        musx::dom::Cmper(7), DifferenceCategory::Differs, 0, 1));
    CHECK_FALSE(classifyLayout(
        "start_meas", {4, 8}, {2, 8}, musx::dom::Cmper(2),
        musx::dom::Cmper(7), DifferenceCategory::Differs, 1, 0));
    CHECK(classifyValues("top", Value(0), Value(-80),
                         DifferenceCategory::Differs, "unmapped") ==
          DifferenceClassification::AwaitsDependentRecovery);
    CHECK(classifyValues("has_staff_scaling", Value(false), Value(true),
                         DifferenceCategory::Differs, "unmapped") ==
          DifferenceClassification::AwaitsDependentRecovery);
    CHECK(classifyValues("distance_to_prev", Value(-72), Value(-80)) ==
          DifferenceClassification::AwaitsDependentRecovery);
    CHECK(classifyValues("distance_to_prev", Value(-72), Value(-80),
                         DifferenceCategory::Differs, "legacy-mus-adjusted") ==
          DifferenceClassification::AwaitsDependentRecovery);
    CHECK_FALSE(classifyValues("top", Value(0), Value(-80)));
    CHECK_FALSE(classifyLayout("end_meas", {3}, {2, 3},
                               musx::dom::Cmper(1), musx::dom::Cmper(2),
                               DifferenceCategory::ReaderOnly));
    CHECK(classifyValues("horz_percent", Value(0.0), Value(107.11),
                         DifferenceCategory::Differs, "unmapped") ==
          DifferenceClassification::FinaleLayoutRecalculation);
    ...
    CHECK_FALSE(classifyValues("top", Value(0), Value(-80),
                               DifferenceCategory::Differs, "unmapped"));
    CHECK_FALSE(classifyValues("has_staff_scaling", Value(false), Value(true),
                               DifferenceCategory::Differs, "unmapped"));
...
TEST_CASE(
    "Controlled fixtures recover staff systems across supported physical "
    "layouts",
    "[class][staff-system]")
{
    struct FixtureCase {
    ...
    for (const auto& expected :
         {FixtureCase{"evidence/F97/Fin97-baseline.mus", 0, 288, -144, 2,
                      111.99},
          FixtureCase{"evidence/F98/F98-baseline.mus", 0, 288, -200, 4, 110.94},
          FixtureCase{"evidence/F2000/F2000-update-layout.mus", 0, 0, -200, 2,
                      331.5},
          FixtureCase{"evidence/F2006/F2006-empty.mus", -463, 144, -200, 2,
                      0.0},
          FixtureCase{"evidence/F2007/F2007-lyric-hyphens.mus", -463, 144, -200,
                      3, 153.25},
          FixtureCase{"evidence/F2012/F2012-baseline.mus", -463, 144, -200, 2,
                      307.17}}) {
        const auto result = readFixture(expected.path);
        const auto system =
            result.document->getOthers()->get<StaffSystem>(0, 1);
        REQUIRE(system);
    ...
    const auto finale97MultipleSystems =
        readFixture("evidence/F97/F97-altnotation.mus");
    const auto laterSystem =
        finale97MultipleSystems.document->getOthers()->get<StaffSystem>(0, 2);
    REQUIRE(laterSystem);
    CHECK(laterSystem->top == 0);
    CHECK(laterSystem->distanceToPrev == -72);

    const auto finale372Compact =
        readFixture("evidence/F372/F372-fileinfo-text.mus");
    const auto finale372System =
        finale372Compact.document->getOthers()->get<StaffSystem>(0, 1);
    ...
    const auto finale372Cruft =
        readFixture("evidence/F372/F372-measure-graphic.mus");
    const auto finale372Last =
        finale372Cruft.document->getOthers()->get<StaffSystem>(0, 3);
    ...
    for (const auto& expected :
         {CodaFixtureCase{"evidence/F100/F100-chg-sys.mus", 389, -247, -299, 2,
                          517, -499, -521, 11, 12, -481},
          CodaFixtureCase{"evidence/F263/F263-chg-sys.mus", 173, -420, -207, 4,
                          641, -34, -244, 16, 19, -151}}) {
        const auto result = readFixture(expected.path);
    ...
    const auto codaSystemOptions =
        readFixture("evidence/F263/F263-sysopts.mus");
    const auto first =
        codaSystemOptions.document->getOthers()->get<StaffSystem>(0, 1);
    const auto second =
        codaSystemOptions.document->getOthers()->get<StaffSystem>(0, 2);
    const auto third =
        codaSystemOptions.document->getOthers()->get<StaffSystem>(0, 3);
    REQUIRE(first);
    REQUIRE(second);
    REQUIRE(third);
    CHECK(first->ssysPercent == 83);
