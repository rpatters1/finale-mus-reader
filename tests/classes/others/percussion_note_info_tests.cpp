...
TEST_CASE("Coverage drops only unreferenced companion percussion maps", "[coverage][percussion]")
{
    ...
    const auto drumStaff = [](std::int64_t mapId) { return Value::Object{{"cmper", 1}, {"which_drum_lib", mapId}}; };
    const auto mapNote = [](std::int64_t mapId) { return Value::Object{{"cmper", mapId}, {"inci", 0}}; };
    ...
    SECTION("active uncompressed map")
    {
        ...
        ComparisonPreparationContext context{source, companion, transformations, FormatEpoch::UncompressedLegacy};
    ...
    SECTION("source-defined map")
    {
        ...
        std::map<ComparisonTransformation, std::uint64_t> transformations;
        ComparisonPreparationContext context{source, companion, transformations, FormatEpoch::DclLegacy};
        runComparisonPreparers(context);
...
TEST_CASE("Coverage aligns upgraded legacy percussion maps through their staffs", "[coverage][percussion]")
{
    using namespace finale_mus_reader::coverage;
    ...
    const auto mapNote = [](std::int64_t mapId, std::int64_t inci, std::int64_t noteType, std::int64_t staffPosition, bool legacy) {
        Value::Object result{{"closed_notehead", 207}, {"cmper", mapId}, {"dwhole_notehead", 250}, {"half_notehead", 250}, {"inci", inci},
            {"perc_note_type", noteType}, {"staff_position", staffPosition}, {"whole_notehead", 250}};
        if (legacy) {
    ...
    const auto compare = [&](SurveySnapshot source, SurveySnapshot companion) {
        ...
        return compareSnapshots(std::move(source), std::move(companion), makePercussionNoteInfoDocument(), makePercussionNoteInfoDocument(),
            FormatEpoch::ZlibLegacy, ByteOrder::BigEndian, nullptr, report);
    };

    SECTION("renumbered map with map-specific note types")
    {
        const auto comparison = compare({{"drum_staff", Value::Array{drumStaff(20, 5, true)}},
                                            {"percussion_note_info", Value::Array{mapNote(5, 0, 28, 2, true), mapNote(5, 1, 27, 4, true)}}},
            {{"drum_staff", Value::Array{drumStaff(20, 1, false)}},
                {"percussion_note_info", Value::Array{mapNote(1, 0, 127, 2, false), mapNote(1, 1, 128, 4, false), mapNote(5, 0, 1, 9, false)}}});
        ...
        CHECK(comparison.expected.at(DifferenceClassification::FinaleReplacedLegacyPercussionMap) == 1);
        CHECK(comparison.transformations.at(ComparisonTransformation::FinaleSynthesizedPercussionMapNote) == 1);
    }

    SECTION("shared legacy map split by staff")
    {
        const auto comparison = compare({{"drum_staff", Value::Array{drumStaff(20, 5, true), drumStaff(21, 5, true)}},
                                            {"percussion_note_info", Value::Array{mapNote(5, 0, 28, 2, true)}}},
            {{"drum_staff", Value::Array{drumStaff(20, 1, false), drumStaff(21, 2, false)}},
                {"percussion_note_info", Value::Array{mapNote(1, 3, 28, 2, false), mapNote(2, 4, 28, 2, false)}}});
        const auto& noteStats = comparison.classes.at("others").at("percussion_note_info");
    ...
    SECTION("map without a companion staff")
    {
        const auto comparison =
            compare({{"drum_staff", Value::Array{drumStaff(20, 5, true)}}, {"percussion_note_info", Value::Array{mapNote(5, 0, 28, 2, true)}}},
                {{"drum_staff", Value::Array{}}, {"percussion_note_info", Value::Array{mapNote(5, 0, 1, 9, false)}}});
        const auto& noteStats = comparison.classes.at("others").at("percussion_note_info");
        CHECK(noteStats.unexpected == 0);
