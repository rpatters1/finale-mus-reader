...
TEST_CASE("Finale conversion decodes a Windows Times stem symbol as MacRoman", "[coverage]")
{
    ...
    ComparisonLeaves source{{symbolPath, {Value(0x00c0), "legacy-mus"}}, {fontPath, {Value("Times"), {}}}};
    ...
    DifferenceContext context{symbolPath, DifferenceCategory::Differs, "legacy-mus", source.at(symbolPath).first, companion.at(symbolPath).first,
        source, companion, finale_mus_reader::FormatEpoch::UncompressedLegacy, finale_mus_reader::ByteOrder::BigEndian, nullptr, report};

    REQUIRE(classifyStemConnectionEncodingError(context) == DifferenceClassification::TextEncodingError);
