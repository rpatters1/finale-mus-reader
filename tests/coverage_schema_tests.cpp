...
TEST_CASE("Deferred measure recovery classifies both directions of a tabled member")
{
    ...
    // The companion states a smart shape the reader cannot yet build, so the value is owed.
    REQUIRE(classifyMeasureDifference(context(smartShape, "legacy-mus", no, yes))
        == DifferenceClassification::AwaitsDependentRecovery);

    // The other direction waits on the same class: with no shapes recovered, the reader cannot
    // know the source bit is stale, so it can neither keep nor clear it on evidence.
    REQUIRE(classifyMeasureDifference(context(smartShape, "legacy-mus", yes, no))
        == DifferenceClassification::AwaitsDependentRecovery);

    // The rest of the "something is attached here" flags are tabled on the same terms, each
    // waiting on the class that holds the objects Finale 27 recomputes it from.
    ...
    // The chord flag is tabled under both provenances it can report. The twelve-word layout
    // carries its bit and reports a stored value; the six-word layout has no word for it.
    for (const auto* origin : {"legacy-mus", "legacy-behavior"}) {
        REQUIRE(classifyMeasureDifference(
                    context("measures[cmper=1].has_chord", origin, no, yes))
            == DifferenceClassification::AwaitsDependentRecovery);
    }
    // A provenance neither layout produces is still not this rule's to absorb.
    REQUIRE_FALSE(classifyMeasureDifference(
        context("measures[cmper=1].has_chord", "unmapped", no, yes)));
    ...
    // The whole deferral backs out through one switch, which is what --strict-deferred sets.
...
TEST_CASE("A later beta's back-save explains a key-signature switch the format cannot carry")
{
    using namespace finale_mus_reader::coverage;

    // A Finale 2012 file whose creator was a Finale 2014 beta: the layout on disk is 17, the
    // release that made the document is 18, and its development status is beta.
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::ZlibLegacy);
    ...
    REQUIRE(classifyMeasureDifference(context(hideAccis))
        == DifferenceClassification::BetaDiscrepancy);
    REQUIRE(classifyMeasureDifference(context(keyless))
        == DifferenceClassification::BetaDiscrepancy);

    // Both halves of the condition are load-bearing. A release build of the same later version
    // wrote what it wrote, so a difference there is not explained away.
    report.creatorVersion = creator(18, 4);
    REQUIRE_FALSE(classifyMeasureDifference(context(hideAccis)));
    // Neither is a beta of the release that owns the format: it had no such member to lose.
...
TEST_CASE("Finale's musx conversion writes a word-extension break the source does not carry")
{
    using namespace finale_mus_reader::coverage;
    finale_mus_reader::ImportReport report(finale_mus_reader::FormatEpoch::DclLegacy);
    const Value no(false);
    ...
    // The sibling leaves the rule reads: the measure's barline and its backwards repeat.
    const auto leaves = [](std::int64_t barline, bool repeat) {
        return ComparisonLeaves{
            {"measures[cmper=1].barline_type", {Value(barline), "legacy-mus"}},
    ...
    // Double, final and solid barlines, and a backwards repeat, are what Finale ties it to.
    ...
    // A measure with none of them is not explained by this rule and stays unexpected.
    REQUIRE_FALSE(classify(leaves(2, false), no, yes));
    REQUIRE_FALSE(classify(leaves(0, false), no, yes));
    // Nor is the other direction: a stored bit the companion drops would be a real loss.
...
TEST_CASE("A composite time-signature word is deferred, a plain one is not")
{
    ...
    const auto classify = [&](std::string_view path, const ComparisonLeaves& side,
        ...
        const DifferenceContext context{path, DifferenceCategory::Differs, "legacy-mus", a, b,
            side, side, finale_mus_reader::FormatEpoch::CodaBanner,
            finale_mus_reader::ByteOrder::BigEndian, nullptr, report};
        return classifyMeasureDifference(context);
    };
    ...
    // A plain beat count or Edu value that disagrees is a decoding failure, not a renumbering.
