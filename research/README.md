...
`tracked-evidence` is the controlled fixture set committed to this repository, surveyed as a corpus in its own right: 56 files, every one of them paired adjacent-exact with a Finale 27 companion. It is tiny and must never be read as a percentage, but it supplies two things the other two cannot. Both of them begin at Finale 1.8.7, so its 19 Finale 1.0.0 fixtures are the **only companion-backed documents of that version in any survey**; and it holds the controlled one-variable edit pairs, which an authored corpus cannot produce. It is also the **only** survey anyone can reproduce: the corpus ships with the repository, so `scripts/inventory.py` run against `tests/evidence/` re-derives this row from a clone with no private configuration. The other two describe private drives and can be re-run only by their owner, so their numbers must be taken on trust in a way this one's need not be.
