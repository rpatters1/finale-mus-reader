...
int main(int argc, char** argv)
{
    ...
    if (raw) {
        for (const auto& block : parsed.blocks) {
            if (!wanted.count(block.info.type)) {
                continue;
            }
    ...
    std::printf("%s: epoch=%s order=%s blocks=%zu\n", argv[1], epochName(parsed.formatEpoch),
        ...
        parsed.blocks.size());
    for (const auto& block : parsed.blocks) {
        std::printf("  type=0x%04x source=0x%zx stored=%zu decoded=%zu%s\n", block.info.type, block.info.sourceOffset, block.info.storedSize,
            block.info.decodedSize, block.info.stored ? " (stored)" : "");
    }
    for (const auto& block : parsed.blocks) {
        if (!all && !wanted.count(block.info.type)) {
            continue;
        }
