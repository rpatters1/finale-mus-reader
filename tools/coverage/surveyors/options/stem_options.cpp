...
std::optional<DifferenceClassification> classifyStemDifference(const DifferenceContext& context)
{
...
Value observeStemOptions(const SurveyContext& ctx)
{
    ...
    for (std::size_t index = 0; index < options->stemConnections.size(); ++index) {
        ...
        // The face as well as the comparator. Finale 27 matches fonts against those
        // installed on the upgrading machine and renumbers its own table
...
COVERAGE_CLASS("options", "stem_options", observeStemOptions, classifyStemDifference);
