...
std::map<std::size_t, Tuple> collectTuples(const finale_mus_reader::ImportReport& report)
{
    ...
    const auto foundInstance = report.fields.find(finale_mus_reader::instanceKey<musx::dom::options::FontOptions>());
    if (foundInstance == report.fields.end()) {
        return result;
    }
    ...
    for (const auto& [name, field] : foundInstance->second) {
        ...
        const auto ordinal = static_cast<std::size_t>(std::stoul(name.substr(prefix.size(), close - prefix.size())));
        const auto member = std::string_view(name).substr(close + 2);
        auto& tuple = result[ordinal];
        if (member == "fontId") {
            tuple.fontId = field;
        } else if (member == "fontSize") {
            tuple.fontSize = field;
        } else if (member == "effects") {
            tuple.effects = field;
        }
    }
    return result;
...
std::size_t decodedFieldOffset(const FieldInfo& field, finale_mus_reader::FormatEpoch epoch, std::size_t ordinal, std::size_t fieldIndex)
{
...
void addSourceField(Value::Object& result, std::string_view name, const FieldInfo& field, finale_mus_reader::FormatEpoch epoch, std::size_t ordinal,
    std::size_t fieldIndex)
{
    result.emplace(name, field.rawValue);
    result.emplace(std::string(name) + "_block_offset", field.blockOffset);
    result.emplace(std::string(name) + "_decoded_field_offset", decodedFieldOffset(field, epoch, ordinal, fieldIndex));
}

Value observeTuple(std::size_t ordinal, const Tuple& tuple, const SurveyContext& ctx)
{
    if (!tuple.fontId || !tuple.fontSize || !tuple.effects) {
        return {};
    }

    const auto options = ctx.document->getOptions()->get<musx::dom::options::FontOptions>();
    const auto actual = options->getFontInfo(static_cast<musx::dom::options::FontOptions::FontType>(ordinal));
    ...
    if (const auto font = ctx.document->getOthers()->get<musx::dom::others::FontDefinition>(musx::dom::SCORE_PARTID, fontId)) {
        if (fontId != 0) {
            fontStatus = "resolved";
        }
        fontName = font->name;
    ...
    result.insert({{"font_id", fontId}, {"font_size", actual->fontSize}, {"effects", actual->getEnigmaStyles()}, {"bold", actual->bold},
        {"italic", actual->italic}, {"underline", actual->underline}, {"strikeout", actual->strikeout}, {"absolute", actual->absolute},
        {"hidden", actual->hidden}, {"origin_bold", std::string(originName(tuple.effects->origin))},
        {"origin_italic", std::string(originName(tuple.effects->origin))}, {"origin_underline", std::string(originName(tuple.effects->origin))},
        {"origin_strikeout", std::string(originName(tuple.effects->origin))}, {"origin_absolute", std::string(originName(tuple.effects->origin))},
        {"origin_hidden", std::string(originName(tuple.effects->origin))}, {"font_id_origin", std::string(originName(tuple.fontId->origin))},
        {"font_size_origin", std::string(originName(tuple.fontSize->origin))}, {"effects_origin", std::string(originName(tuple.effects->origin))},
        {"font_status", fontStatus}, {"font_name", fontName}, {"normalized_font_name", musx::dom::normalizeFontName(fontName)}});
    return result;
}
...
Value observeFontOptions(const SurveyContext& ctx)
{
    ...
    for (const auto& [type, font] : options->fontOptions) {
        ...
        if (font->fontId != 0 && !ctx.document->getOthers()->get<musx::dom::others::FontDefinition>(musx::dom::SCORE_PARTID, font->fontId)) {
    ...
    return Value::Object{{"option_count", options->fontOptions.size()}, {"recovered_count", recoveredCount}, {"legacy_behavior_count", behaviorCount},
        {"default_count", defaultCount}, {"unmapped_count", unmappedCount}, {"musx_only_count", musxOnlyCount},
