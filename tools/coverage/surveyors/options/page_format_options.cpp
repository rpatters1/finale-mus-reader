...
Value observePageFormat(const PageFormatTarget& format, const SurveyContext& ctx, std::string_view memberPrefix)
{
    auto result = observe(format, ctx, field("page_height", &PageFormatTarget::pageHeight), field("page_width", &PageFormatTarget::pageWidth),
        field("page_percent", &PageFormatTarget::pagePercent), field("sys_percent", &PageFormatTarget::sysPercent),
        field("raw_staff_height", &PageFormatTarget::rawStaffHeight), field("left_page_margin_top", &PageFormatTarget::leftPageMarginTop),
        ...
        field("right_page_margin_right", &PageFormatTarget::rightPageMarginRight), field("sys_margin_top", &PageFormatTarget::sysMarginTop),
        field("sys_margin_left", &PageFormatTarget::sysMarginLeft), field("sys_margin_bottom", &PageFormatTarget::sysMarginBottom),
        field("sys_margin_right", &PageFormatTarget::sysMarginRight), field("sys_distance_between", &PageFormatTarget::sysDistanceBetween),
        field("first_page_margin_top", &PageFormatTarget::firstPageMarginTop), field("first_sys_margin_top", &PageFormatTarget::firstSysMarginTop),
        field("first_sys_margin_left", &PageFormatTarget::firstSysMarginLeft),
        field("first_sys_margin_distance", &PageFormatTarget::firstSysMarginDistance), field("facing_pages", &PageFormatTarget::facingPages),
...
#define ADD_PAGE_FORMAT_ORIGIN(member) \
    fields.emplace("origin_" #member, fieldOrigin<PageFormatOptionsTarget>(ctx, std::string(memberPrefix).append(#member)))
...
Value observePageFormatOptions(const SurveyContext& ctx)
{
    ...
    if (!options || !options->pageFormatScore || !options->pageFormatParts) {
        return {};
    }

    auto result = observe(*options, ctx, field("adjust_page_scope", &PageFormatOptionsTarget::adjustPageScope),
        field("avoid_system_margin_collisions", &PageFormatOptionsTarget::avoidSystemMarginCollisions),
        originField<PageFormatOptionsTarget>("origin_adjustPageScope", "adjustPageScope"),
        originField<PageFormatOptionsTarget>("origin_avoidSystemMarginCollisions", "avoidSystemMarginCollisions"));
    auto& fields = result.asObject();
    fields.emplace("page_format_score", observePageFormat(*options->pageFormatScore, ctx, "pageFormatScore."));
    fields.emplace("page_format_parts", observePageFormat(*options->pageFormatParts, ctx, "pageFormatParts."));
    return result;
}

COVERAGE_CLASS("options", "page_format_options", observePageFormatOptions, classifyPageFormatOptionsDifference);
