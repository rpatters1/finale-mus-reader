...
classifyMusicSpacingDifference(const DifferenceContext& context)
{
    using enum DifferenceCategory;
    if (context.category != Differs ||
        !comparisonPathStartsWith(context.path, "music_spacing_options.")) {
        return std::nullopt;
    }
...
Value observeMusicSpacingOptions(const SurveyContext& ctx)
{
    ...
    if (!options) return {};
    return observe(*options, ctx, field("min_width", &Target::minWidth),
                   field("max_width", &Target::maxWidth),
...
COVERAGE_CLASS("options", "music_spacing_options", observeMusicSpacingOptions,
               classifyMusicSpacingDifference);
