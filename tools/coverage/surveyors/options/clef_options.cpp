...
std::optional<DifferenceClassification> classifyClefDifference(const DifferenceContext& context)
{
    ...
    if (context.category == Differs && context.origin == "finale27-default" && comparisonPathEndsWith(context.path, "shape_id")) {
...
Value observeClefOptions(const SurveyContext& ctx)
{
    ...
    auto result = observe(*options, ctx, field("default_clef", &Target::defaultClef), field("clef_change_percent", &Target::clefChangePercent),
        ...
        field("clef_time_separ", &Target::clefTimeSepar), field("show_clef_first_system_only", &Target::showClefFirstSystemOnly),
        field("cautionary_clef_changes", &Target::cautionaryClefChanges), originField<Target>("origin_defaultClef", "defaultClef"),
        originField<Target>("origin_clefChangePercent", "clefChangePercent"), originField<Target>("origin_clefChangeOffset", "clefChangeOffset"),
        originField<Target>("origin_clefFrontSepar", "clefFrontSepar"), originField<Target>("origin_clefBackSepar", "clefBackSepar"),
        originField<Target>("origin_clefKeySepar", "clefKeySepar"), originField<Target>("origin_clefTimeSepar", "clefTimeSepar"),
    ...
    for (std::size_t index = 0; index < options->clefDefs.size(); ++index) {
        ...
        if (def->useOwnFont && def->font) {
            if (const auto font = ctx.document->getOthers()->get<musx::dom::others::FontDefinition>(musx::dom::SCORE_PARTID, def->font->fontId)) {
                fontName = font->name;
            }
        }
        ...
        if (def->isShape && def->shapeId != 0) {
            danglingShape = !ctx.document->getOthers()->get<musx::dom::others::ShapeDef>(musx::dom::SCORE_PARTID, def->shapeId);
        }
        definitions.emplace_back(Value::Object{{"index", index}, {"middle_c_pos", def->middleCPos},
            ...
            {"scale_to_staff_height", def->scaleToStaffHeight}, {"use_own_font", def->useOwnFont}, {"font_id", def->font ? def->font->fontId : 0},
            {"font_size", def->font ? def->font->fontSize : 0}, {"font_bold", def->font && def->font->bold},
            {"font_italic", def->font && def->font->italic}, {"font_underline", def->font && def->font->underline},
            {"font_strikeout", def->font && def->font->strikeout}, {"font_absolute", def->font && def->font->absolute},
            {"font_hidden", def->font && def->font->hidden}, {"origin_isShape", fieldOrigin<Target>(ctx, prefix + "flags")},
            {"origin_scaleToStaffHeight", fieldOrigin<Target>(ctx, prefix + "flags")},
            {"origin_useOwnFont", fieldOrigin<Target>(ctx, prefix + "flags")}, {"origin_fontId", fieldOrigin<Target>(ctx, prefix + "font.fontId")},
            ...
            {"origin_fontHidden", fieldOrigin<Target>(ctx, prefix + "font.effects")}, {"font_name", fontName}, {"dangling_shape", danglingShape},
