...
inline bool sourceHasNoteAttachedItemsExpansion(const DifferenceContext& context, std::string_view objectPath)
{
    ...
    for (const auto& expected : expectedLeaves) {
        ...
        if (source == context.source.end() || !source->second.first.isBool() || !source->second.first.asBool()
            || source->second.second != expected.sourceOrigin) {
            return false;
        }
    }
    return true;
...
inline std::optional<DifferenceClassification> classifyNoteAttachedItemsExpressionUpgradeLoss(
    const DifferenceContext& context, std::string_view classPrefix)
{
    ...
    if (context.category != DifferenceCategory::Differs || context.origin != "finale27-default"
        || !comparisonPathStartsWith(context.path, classPrefix) || !comparisonPathEndsWith(context.path, expressionSuffix)
        || !context.sourceValue.isBool() || context.sourceValue.asBool() || !context.companionValue.isBool() || !context.companionValue.asBool()
        || !sourceAtOrAfter(context.epoch, context.sourceVersion, FormatEpoch::UncompressedLegacy, versions::finale2000)) {
        return std::nullopt;
    }

    const auto objectPath = context.path.substr(0, context.path.size() - expressionSuffix.size());
    return sourceHasNoteAttachedItemsExpansion(context, objectPath) ? std::optional{DifferenceClassification::FinaleUpgradeLoss} : std::nullopt;
}

inline std::optional<DifferenceClassification> classifyAggregateOtherSmartShapeUpgradeLoss(
    const DifferenceContext& context, std::string_view classPrefix, bool usesAggregateOtherAttachedItems)
{
    if (context.category == DifferenceCategory::Differs && context.origin == "legacy-mus" && comparisonPathStartsWith(context.path, classPrefix)
        && comparisonPathEndsWith(context.path, ".alt_hide_other_smart_shapes") && context.sourceValue.isBool() && context.companionValue.isBool()
        && context.sourceValue.asBool() != context.companionValue.asBool() && usesAggregateOtherAttachedItems) {
        return DifferenceClassification::FinaleUpgradeLoss;
    }
    return std::nullopt;
}

inline std::optional<DifferenceClassification> classifyNoteAttachedItemsAggregateHideUpgradeLoss(const DifferenceContext& context,
    std::string_view classPrefix, std::string_view maskContainer, std::optional<std::string_view> requiredMaskOrigin = {})
{
    constexpr std::string_view aggregateHideSuffixes[] = {".hide_chords", ".hide_fretboards"};
    const auto suffix =
        std::ranges::find_if(aggregateHideSuffixes, [&](const auto candidate) { return comparisonPathEndsWith(context.path, candidate); });
    if (suffix == std::ranges::end(aggregateHideSuffixes) || context.category != DifferenceCategory::Differs || context.origin != "legacy-mus"
        || !comparisonPathStartsWith(context.path, classPrefix) || !context.sourceValue.isBool() || !context.sourceValue.asBool()
        || !context.companionValue.isBool() || context.companionValue.asBool()
        || !sourceAtOrAfter(context.epoch, context.sourceVersion, FormatEpoch::UncompressedLegacy, versions::finale2000)
        || !sourcePredatesVersion(context.epoch, context.sourceVersion, FormatEpoch::ZlibLegacy, versions::finale2009)) {
        return std::nullopt;
    }
    ...
    const auto mask = context.source.find(std::string(objectPath) + std::string(maskContainer) + std::string(suffix->substr(1)));
    if (mask == context.source.end() || !mask->second.first.isBool() || !mask->second.first.asBool()
...
inline std::optional<DifferenceClassification> classifyDisabledNoteFontSize(const DifferenceContext& context, std::string_view classPrefix)
{
    constexpr std::string_view noteFontSizeSuffix = ".note_font.font_size";
    if (context.category != DifferenceCategory::Differs || !comparisonPathStartsWith(context.path, classPrefix)
    ...
    if (useNoteFont != context.source.end() && useNoteFont->second.first.isBool() && !useNoteFont->second.first.asBool()) {
...
std::string staffLikeOrigin(const Target& staff, const SurveyContext& context, std::string_view member)
{
    return fieldOrigin<Target>(context, member, staff);
}

template <typename Target, typename T>
void addStaffLikeLeaf(Value::Object& object, const Target& staff, const SurveyContext& context, std::string_view reportMember,
    std::string_view originMember, std::string_view leaf, T value)
{
    object.emplace(std::string(leaf), staffLikeValue(value));
    object.emplace("origin_" + std::string(originMember), staffLikeOrigin(staff, context, reportMember));
}

template <typename Target, typename T>
void addStaffLikeLeaf(
    Value::Object& object, const Target& staff, const SurveyContext& context, std::string_view member, std::string_view leaf, T value)
{
...
Value::Object observeStaffLike(const Target& staff, const SurveyContext& context)
{
    ...
    const auto instance = finale_mus_reader::instanceKey<Target>(staff.getSourcePartId(), staff.getCmper());
    if (const auto* origin = context.report.findInstanceOrigin(instance)) {
        result.emplace("origin", originName(*origin));
    }

#define STAFF_LIKE_LEAF(member, leaf) addStaffLikeLeaf(result, staff, context, #member, #leaf, staff.member)
    ...
    addStaffLikeLeaf(result, staff, context, "staffLines", "staff_lines", staff.staffLines.value_or(0));
...
#define STAFF_LIKE_FONT_LEAF(member, leaf) \
    addStaffLikeLeaf(noteFont, staff, context, "noteFont." #member, #member, #leaf, font ? font->member : decltype(font->member){})
...
#undef STAFF_LIKE_FONT_LEAF
    ...
    addStaffLikeLeaf(transposition, staff, context, "transposition.setToClef", "setToClef", "set_to_clef", trans ? trans->setToClef : false);
    addStaffLikeLeaf(
    ...
    addStaffLikeLeaf(keysig, staff, context, "transposition.keysig.adjust", "adjust", "adjust", trans && trans->keysig ? trans->keysig->adjust : 0);
    keysig.emplace("present", bool(trans && trans->keysig));
    transposition.emplace("keysig", std::move(keysig));
    Value::Object chromatic;
    addStaffLikeLeaf(chromatic, staff, context, "transposition.chromatic.alteration", "alteration", "alteration",
        trans && trans->chromatic ? trans->chromatic->alteration : 0);
    addStaffLikeLeaf(chromatic, staff, context, "transposition.chromatic.diatonic", "diatonic", "diatonic",
