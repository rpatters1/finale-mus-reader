...
Value observeGraphicAssignments(const SurveyContext& ctx)
{
    ...
    for (const auto& assign : sourceInstances<Target>(ctx)) {
        auto observed = observe(*assign, ctx, field("cmper", [](const Target& value) { return value.getCmper(); }),
            ...
            field("v_align", &Target::vAlign), field("fixed_perc", &Target::fixedPerc), field("saved_record", &Target::savedRecord),
            field("orig_width", &Target::origWidth), field("orig_height", &Target::origHeight), field("graphic_cmper", &Target::graphicCmper));
        if constexpr (std::is_same_v<Target, musx::dom::others::PageGraphicAssign>) {
            observed.asObject().insert({{"display_type", static_cast<std::int64_t>(assign->displayType)},
                {"pos_from", static_cast<std::int64_t>(assign->posFrom)}, {"start_page", assign->startPage}, {"end_page", assign->endPage},
                ...
                {"right_pg_pos_from", static_cast<std::int64_t>(assign->rightPgPosFrom)}, {"right_pg_fixed_perc", assign->rightPgFixedPerc},
                {"right_pg_left", assign->rightPgLeft}, {"right_pg_bottom", assign->rightPgBottom}});
        }
