...
std::optional<std::int64_t> staffStyleAssignIntegerMember(const Value& value, std::string_view member)
{
    if (!value.isObject()) {
        return std::nullopt;
    }
...
std::optional<std::int64_t> staffStyleAssignIntegerMember(const Value::Object& value, std::string_view member)
{
    const auto found = value.find(member);
    return found != value.end() && found->second.isInteger() ? std::optional{found->second.asInteger()} : std::nullopt;
}

using StaffStyleAssignRange = std::tuple<std::int64_t, std::int64_t, std::int64_t, std::int64_t, std::int64_t, std::int64_t>;

std::optional<StaffStyleAssignRange> staffStyleAssignRange(const Value& value)
{
    std::array<std::int64_t, 6> fields{};
    std::size_t index{};
    for (const auto member : {"part_id", "cmper", "start_meas", "start_edu", "end_meas", "end_edu"}) {
...
std::string staffStyleAssignRangeKey(const StaffStyleAssignRange& range)
{
    ...
    return "part=" + std::to_string(part) + ",staff=" + std::to_string(staff) + ",range=" + std::to_string(startMeasure) + ':'
           + std::to_string(startEdu) + '-' + std::to_string(endMeasure) + ':' + std::to_string(endEdu);
}

void copyStaffStyleAssignMember(const Value::Object& source, Value::Object& destination, std::string_view member)
{
...
Value::Object* staffStyleAssignBaseStaff(SurveySnapshot& snapshot, std::int64_t partId, std::int64_t staffId, bool scoreFallback = true)
{
    ...
    const auto findPart = [&](std::int64_t candidatePart) -> Value::Object* {
        for (auto& staff : found->second.asArray()) {
            if (staffStyleAssignIntegerMember(staff, "part_id") == candidatePart && staffStyleAssignIntegerMember(staff, "cmper") == staffId) {
    ...
    return scoreFallback && partId != musx::dom::SCORE_PARTID ? findPart(musx::dom::SCORE_PARTID) : nullptr;
}
...
StaffStyleAssignBaseLeaf staffStyleAssignBaseLeaf(const Value::Object& base, std::string_view path)
{
    ...
    while (begin < path.size()) {
        ...
        const auto leaf = path.substr(begin, end == std::string_view::npos ? path.size() - begin : end - begin);
        ...
        if (end == std::string_view::npos) {
            ...
            return {&found->second, origin != object->end() && origin->second.isString() ? &origin->second : nullptr};
        }
...
void staffStyleAssignCompleteFromBase(Value::Object& effective, const Value::Object* base, std::string_view field)
{
...
void completeStaffStyleAssignGroupsFromBase(SurveySnapshot& source, SurveySnapshot& companion)
{
    ...
    for (const auto& [key, sourceGroup] : sourceGroups) {
        ...
        if (sourceApplied == sourceGroup->end() || companionApplied == companionFound->second->end() || !sourceApplied->second.isObject()
            || !companionApplied->second.isObject()) {
            continue;
        }
    ...
    const auto groupAssignments = [](SurveySnapshot& snapshot) {
        ...
        for (auto& [range, assignments] : groups) {
            ...
            std::ranges::stable_sort(assignments, [](const Value* left, const Value* right) {
                return staffStyleAssignIntegerMember(*left, "inci").value_or(0) < staffStyleAssignIntegerMember(*right, "inci").value_or(0);
            });

            const auto& first = assignments.front()->asObject();
            Value::Object group;
            for (const auto member : {"part_id", "share_mode", "origin", "cmper", "start_meas", "start_edu", "end_meas", "end_edu",
            ...
            effective.emplace(std::string(staff_style_semantics::classifierAssignmentCount), static_cast<std::int64_t>(assignments.size()));
...
std::optional<DifferenceClassification> classifyStaffStyleAssignAttachedItemsUpgrade(const DifferenceContext& context)
{
    constexpr std::string_view classPrefix = "staff_style_assigns[";
    if (const auto expression = staff_fields::classifyNoteAttachedItemsExpressionUpgradeLoss(context, classPrefix)) {
        return expression;
    }
    ...
    const auto aggregateOther =
        context.source.find(std::string(prefix) + "." + std::string(staff_style_semantics::classifierAggregateOtherAttachedItems));
    const auto usesAggregateOther =
        aggregateOther != context.source.end() && aggregateOther->second.first.isBool() && aggregateOther->second.first.asBool();
    if (const auto smartShapes = staff_fields::classifyAggregateOtherSmartShapeUpgradeLoss(context, classPrefix, usesAggregateOther)) {
        return smartShapes;
    }
...
std::optional<DifferenceClassification> classifyStaffStyleAssignInstrumentPromotion(const DifferenceContext& context)
{
    const auto prefix = staffStyleAssignAppliedStylePrefix(context.path);
    ...
    const auto sourceCount =
        comparisonIntegerLeaf(context.source, std::string(prefix) + "." + std::string(staff_style_semantics::classifierAssignmentCount));
    const auto companionCount =
        comparisonIntegerLeaf(context.companion, std::string(prefix) + "." + std::string(staff_style_semantics::classifierAssignmentCount));
    ...
    const auto sourceNotation = comparisonIntegerLeaf(context.source, std::string(prefix) + ".notation_style");
    const auto companionNotation = comparisonIntegerLeaf(context.companion, std::string(prefix) + ".notation_style");
    const auto percussion = static_cast<std::int64_t>(staff_style_semantics::SurveyTarget::NotationStyle::Percussion);
    const auto sourceAndCompanionArePercussion =
        sourceNotation && *sourceNotation == percussion && companionNotation && *companionNotation == percussion;
    const auto sourceMaskActiveAndCompanionMaskInactive = [&](std::string_view maskSuffix) {
        return staff_style_semantics::hasActiveInstrumentMask(context.source, prefix, maskSuffix, staff_style_semantics::classifierMaskPrefix)
               && !staff_style_semantics::hasActiveInstrumentMask(context.companion, prefix, maskSuffix, staff_style_semantics::classifierMaskPrefix);
    };
    const auto companionIsCompletePercussionInstrument =
        sourceAndCompanionArePercussion
        && staff_style_semantics::hasAnyInstrumentOnlyMask(context.companion, prefix, staff_style_semantics::classifierMaskPrefix)
        && staff_style_semantics::hasAllRequiredInstrumentMasks(context.companion, prefix, staff_style_semantics::classifierMaskPrefix);
    if (context.origin == "legacy-mus" && companionIsCompletePercussionInstrument && mask && sourceMaskActiveAndCompanionMaskInactive(*mask)) {
        if (*mask == ".masks.float_notehead_font") {
            return DifferenceClassification::FinaleUpgradeNormalization;
        }
        if (*mask == ".masks.no_key" && context.sourceValue.isBool() && context.sourceValue.asBool() && context.companionValue.isBool()
            && !context.companionValue.asBool()) {
            return DifferenceClassification::FinaleUpgradeNormalization;
        }
    }
    const auto instrumentPromotion =
        staff_style_semantics::hasAnyInstrumentOnlyMask(context.source, prefix, staff_style_semantics::classifierMaskPrefix)
        && !staff_style_semantics::hasAllRequiredInstrumentMasks(context.source, prefix, staff_style_semantics::classifierMaskPrefix)
        && staff_style_semantics::hasAnyInstrumentOnlyMask(context.companion, prefix, staff_style_semantics::classifierMaskPrefix)
        && staff_style_semantics::hasAllRequiredInstrumentMasks(context.companion, prefix, staff_style_semantics::classifierMaskPrefix);
    const auto expandedSplit = *companionCount > *sourceCount;
    const auto oneForOnePromotion = context.origin == "legacy-mus" && *sourceCount == 1 && *companionCount == 1 && instrumentPromotion;
    ...
    if (context.origin == "legacy-mus" && expandedSplit && instrumentPromotion && relativePath == ".no_key" && context.sourceValue.isBool()
        && context.sourceValue.asBool() && context.companionValue.isBool() && !context.companionValue.asBool()
        && staff_style_semantics::hasActiveMask(context.source, prefix, "." + std::string(staff_style_semantics::classifierMaskPrefix) + "no_key")
        && !staff_style_semantics::hasActiveMask(
            context.companion, prefix, "." + std::string(staff_style_semantics::classifierMaskPrefix) + "no_key")) {
    ...
    return companionNotation && *companionNotation == percussion ? std::optional{DifferenceClassification::FinaleUpgradeNormalization} : std::nullopt;
}

std::optional<DifferenceClassification> classifyStaffStyleAssignDifference(const DifferenceContext& context)
{
...
Value observeStaffStyleAssignments(const SurveyContext& context)
{
    ...
    for (const auto& assign : sourceInstances<Target>(context)) {
        auto object = observe(*assign, context, field("cmper", [](const Target& value) { return value.getCmper(); }),
            field("inci", [](const Target& value) { return value.getInci().value_or(0); }), field("style_id", &Target::styleId),
            field("start_meas", &Target::startMeas), field("start_edu", &Target::startEdu), field("end_meas", &Target::endMeas),
            field("end_edu", &Target::endEdu), field("origin_styleId", staffStyleAssignOrigin("styleId")),
            field("origin_startMeas", staffStyleAssignOrigin("startMeas")), field("origin_startEdu", staffStyleAssignOrigin("startEdu")),
            field("origin_endMeas", staffStyleAssignOrigin("endMeas")), field("origin_endEdu", staffStyleAssignOrigin("endEdu")));
        if (const auto style = assign->getStaffStyle()) {
            object.asObject().emplace(
                staffStyleAssignPatchKey, staff_style_semantics::canonicalPatch(staff_style_semantics::observe(*style, context),
                                              staff_style_semantics::usesAggregateOtherAttachedItems(context.report, style->getCmper())));
        } else {
            object.asObject().emplace(staffStyleAssignPatchKey, Value::Object{{"unresolved_style_id", Value(assign->styleId)}});
        }
...
COVERAGE_CLASS_WITH_PREPARATION(
    "others", staffStyleAssignCoverageKey, observeStaffStyleAssignments, classifyStaffStyleAssignDifference, prepareStaffStyleAssignComparison);
