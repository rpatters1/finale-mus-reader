...
void collectPercussionReferencedMapIds(const SurveySnapshot& snapshot, std::set<std::int64_t>& result)
{
...
std::size_t dropUnreferencedCompanionMapNotes(const SurveySnapshot& source, SurveySnapshot& companion)
{
...
std::map<PercussionMapKey, std::set<std::int64_t>> percussionMapTargets(const SurveySnapshot& source, const SurveySnapshot& companion)
{
...
void realignLegacyPercussionMapNotes(SurveySnapshot& source, const SurveySnapshot& companion)
{
    ...
    if (sourceFound == source.end() || !sourceFound->second.isArray() || companionFound == companion.end() || !companionFound->second.isArray()) {
        return;
    }
    ...
    for (const auto& note : sourceFound->second.asArray()) {
        ...
        for (const auto targetMap : foundTargets->second) {
            ...
            if (foundTargets->second.size() == 1 && sourceInci && sourceMapIncis != sourceIncis.end() && targetMapIncis != companionMapIncis.end()
                ...
                aligned.asObject()["inci"] = Value(targetMapIncis->second.at(static_cast<std::size_t>(position - sourceMapIncis->second.begin())));
            ...
            if (sourceTypeCounts[sourceRow] == 1 && companionIncidence != companionIncis.end() && companionIncidence->second.size() == 1) {
...
void preparePercussionNoteInfoComparison(ComparisonPreparationContext& context)
{
    ...
    if (dropped) {
        context.transformations[ComparisonTransformation::FinaleSynthesizedPercussionMapNote] += dropped;
    }
}
...
Value observePercussionNoteInfo(const SurveyContext& ctx)
{
    ...
    for (const auto& note : sourceInstances<Target>(ctx)) {
        const auto origin = [&ctx, &note](std::string_view member) { return fieldOrigin<Target>(ctx, member, *note); };
        result.emplace_back(
            observe(*note, ctx, field("closed_notehead", [](const Target& value) { return static_cast<std::uint32_t>(value.closedNotehead); }),
                field("cmper", [](const Target& value) { return value.getCmper(); }),
                field("dwhole_notehead", [](const Target& value) { return static_cast<std::uint32_t>(value.dwholeNotehead); }),
                field("half_notehead", [](const Target& value) { return static_cast<std::uint32_t>(value.halfNotehead); }),
                field("inci", [](const Target& value) { return value.getInci().value_or(0); }),
                field("origin_closedNotehead", [&origin](const Target&) { return origin("closedNotehead"); }),
                field("origin_dwholeNotehead", [&origin](const Target&) { return origin("dwholeNotehead"); }),
                field("origin_halfNotehead", [&origin](const Target&) { return origin("halfNotehead"); }),
                field("origin_percNoteType", [&origin](const Target&) { return origin("percNoteType"); }),
                field("origin_staffPosition", [&origin](const Target&) { return origin("staffPosition"); }),
                field("origin_wholeNotehead", [&origin](const Target&) { return origin("wholeNotehead"); }),
                field("perc_note_type", &Target::percNoteType), field("staff_position", &Target::staffPosition),
                field("whole_notehead", [](const Target& value) { return static_cast<std::uint32_t>(value.wholeNotehead); })));
    }
    return result;
}

COVERAGE_CLASS_WITH_PREPARATION("others", "percussion_note_info", observePercussionNoteInfo, nullptr, preparePercussionNoteInfoComparison);
