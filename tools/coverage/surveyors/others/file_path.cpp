...
std::optional<DifferenceClassification> classifyEmbeddedLocator(
    const DifferenceContext& context, std::string_view locatorPath, std::string_view classKey, std::string_view member)
{
    ...
    for (const auto& [path, leaf] : source) {
        if (!path.ends_with(descMember) || !leaf.first.isInteger() || leaf.first.asInteger() == 0
            || !(path.starts_with("page_graphic_assigns[") || path.starts_with("shape_graphic_assigns[")
                 || path.starts_with("meas_graphic_assigns["))) {
            continue;
        }
        ...
        if (cmper == std::string::npos) {
            continue;
        }
        const auto part = path.substr(identity, cmper - identity);
        const auto descriptionKey = [&](std::string_view partPrefix) {
            return std::string(fileDescriptionsCoverageKey) + "[" + std::string(partPrefix) + "cmper=" + std::to_string(leaf.first.asInteger())
                   + "].path_id";
        };
        auto pathId = !resolvePath ? std::optional{leaf.first.asInteger()} : comparisonIntegerLeaf(source, descriptionKey(part));
        ...
        const auto pathKey = [&](std::string_view partPrefix) {
            return std::string(classKey) + "[" + std::string(partPrefix) + "cmper=" + std::to_string(*pathId) + "]." + std::string(member);
        };
        auto targetPath = pathKey(part);
        if (!source.contains(targetPath) && !part.empty()) {
            targetPath = pathKey({});
        }
        if (targetPath != locatorPath) {
            continue;
        }
    ...
    return embedded ? std::optional{DifferenceClassification::FinaleUpgradeNormalization} : std::nullopt;
}
...
std::optional<DifferenceClassification> classifyEmbeddedFilePath(const DifferenceContext& context)
{
    if (context.category != DifferenceCategory::Differs || context.origin != "legacy-mus"
        || !context.path.starts_with(std::string(filePathsCoverageKey) + "[") || !context.path.ends_with(".path") || !context.sourceValue.isString()
...
std::optional<DifferenceClassification> classifySynthesizedFileLocator(
    const DifferenceContext& context, std::string_view classKey, std::string_view blobMember)
{
    if (context.category != DifferenceCategory::CompanionOnly || !context.origin.empty() || !context.path.starts_with(std::string(classKey) + "[")) {
        return std::nullopt;
    }
    ...
    if (member != "share_mode" && member != "length" && member != blobMember) {
        return std::nullopt;
    }
    ...
    if (classKey == fileUrlBookmarksCoverageKey) {
        ...
        if (comparisonIntegerLeaf(source, typePath) != static_cast<std::int64_t>(PathType::DosPath)
            ...
            || source.at(typePath).second != "legacy-mus" || !classifyEmbeddedLocator(context, typePath, fileDescriptionsCoverageKey, "path_type")) {
            return std::nullopt;
        }
    }
...
std::optional<DifferenceClassification> classifyEmbeddedFileAlias(const DifferenceContext& context)
{
    ...
    if (context.category != DifferenceCategory::Differs || !context.path.starts_with(std::string(fileAliasesCoverageKey) + "[")) {
        return std::nullopt;
    }
    const auto end = context.path.find("].");
    if (end == std::string_view::npos) {
    ...
    if (!comparisonIntegerLeaf(context.source, lengthPath) || !comparisonIntegerLeaf(context.companion, lengthPath)) {
        return std::nullopt;
    }
...
std::optional<DifferenceClassification> classifyEmbeddedFileDescription(const DifferenceContext& context)
{
    using PathType = musx::dom::others::FileDescription::PathType;
    if (context.category != DifferenceCategory::Differs || context.origin != "legacy-mus"
        || !context.path.starts_with(std::string(fileDescriptionsCoverageKey) + "[") || !context.sourceValue.isInteger()
...
Value observeFileLocators(const SurveyContext& ctx, Populate populate)
{
    ...
    for (const auto& target : sourceInstances<Target>(ctx)) {
        auto value = observe(*target, ctx, field("cmper", [](const Target& item) { return item.getCmper(); }));
        const auto member = [&](const char* name, const char* originName, auto accessor) {
            ...
            object.emplace(std::string("origin_") + originName, fieldOrigin<Target>(ctx, originName, *target));
        };
        populate(member);
...
Value observeFilePaths(const SurveyContext& ctx)
{
    ...
    return observeFileLocators<Target>(ctx, [](auto member) { member("path", "path", &Target::path); });
}
