...
bool isChordSuffixFiller(const Value& element)
{
    ...
    return chordSuffixIntegerMember(element, "symbol") == 0 && chordSuffixIntegerMember(element, "xdisp") == 0
           && chordSuffixIntegerMember(element, "ydisp") == 0 && chordSuffixIntegerMember(element, "prefix") == 0
           && chordSuffixBoolMemberIs(element, "is_number", false) && chordSuffixIntegerMember(*font, "font_id") == 0
           && chordSuffixIntegerMember(*font, "font_size") == 24 && chordSuffixBoolMemberIs(*font, "bold", false)
           && chordSuffixBoolMemberIs(*font, "italic", false) && chordSuffixBoolMemberIs(*font, "underline", false)
           && chordSuffixBoolMemberIs(*font, "strikeout", false) && chordSuffixBoolMemberIs(*font, "absolute", false)
...
std::size_t removeFinaleAddedChordSuffixFillers(SurveySnapshot& source, SurveySnapshot& companion)
{
    ...
    for (const auto& element : companionFound->second.asArray()) {
        ...
        const auto inci = chordSuffixIntegerMember(element, "inci");
        if (identity && inci) {
            companionLastIncidence[*identity] = (std::max)(companionLastIncidence[*identity], *inci);
        }
    }

    auto& elements = companionFound->second.asArray();
    const auto oldSize = elements.size();
    std::erase_if(elements, [&](const Value& element) {
        const auto identity = chordSuffixIdentity(element);
        const auto inci = chordSuffixIntegerMember(element, "inci");
        if (!identity || !inci || !isChordSuffixFiller(element)) {
            return false;
        }
        const auto sourceLast = sourceLastIncidence.find(*identity);
        const auto companionLast = companionLastIncidence.find(*identity);
        return sourceLast != sourceLastIncidence.end() && companionLast != companionLastIncidence.end() && *inci == sourceLast->second + 1
               && *inci == companionLast->second;
    });
...
{
    if (context.sourceEpoch != finale_mus_reader::FormatEpoch::CodaBanner) {
        return;
    }
...
bool numericOnlySuffixSemanticsWereLost(const ComparisonLeaves& source, const ComparisonLeaves& companion, const std::string& elementPath)
{
    ...
    if (sourceNumber == source.end() || !sourceNumber->second.first.isBool() || !sourceNumber->second.first.asBool()
        || companionNumber == companion.end() || !companionNumber->second.first.isBool() || companionNumber->second.first.asBool()
        || sourcePrefix == source.end() || !sourcePrefix->second.first.isInteger() || companionPrefix == companion.end()
        || !companionPrefix->second.first.isInteger() || companionPrefix->second.first.asInteger() != 0) {
        return false;
    }

    const auto inciAt = elementPath.rfind(",inci=");
    if (inciAt == std::string::npos) {
        return false;
    }
    ...
    for (const auto& [path, unused] : source) {
        if (path.rfind(definitionPrefix, 0) != 0) {
            continue;
        }
        const auto close = path.find(']', definitionPrefix.size());
        if (close != std::string::npos) {
            sourceElements.insert(path.substr(0, close + 1));
        }
    }
    if (sourceElements != std::set<std::string>{elementPath}) {
        return false;
    }

    const auto sameExceptNumberAndPrefix = [&](const ComparisonLeaves& left, const ComparisonLeaves& right) {
        for (const auto& [path, value] : left) {
            if (path.rfind(elementPath + ".", 0) != 0 || path == elementPath + ".is_number" || path == elementPath + ".prefix") {
                continue;
            }
            const auto found = right.find(path);
            if (found == right.end() || found->second.first != value.first) {
                return false;
            }
        }
        return true;
    };
    return sameExceptNumberAndPrefix(source, companion) && sameExceptNumberAndPrefix(companion, source);
}

std::optional<DifferenceClassification> classifyChordSuffixElementDifference(const DifferenceContext& context)
{
    ...
    static const std::regex changedField(R"(^(chord_suffix_elements\[(?:part_id=\d+,)?cmper=\d+,inci=\d+\])\.(is_number|prefix)$)");
    ...
    if (context.category == Differs && context.origin == "legacy-mus"
        ...
        if (numericOnlySuffixSemanticsWereLost(context.source, context.companion, elementPath)) {
            if (std::string_view(match[2].first, match[2].second) == "prefix") {
                ...
                if (sourcePrefix->second.first.asInteger() == 0) {
                    return std::nullopt;
                }
            }
...
Value observeChordSuffixElements(const SurveyContext& ctx)
{
    ...
    for (const auto& element : sourceInstances<Target>(ctx)) {
        const auto origin = [&ctx, &element](std::string_view member) { return fieldOrigin<Target>(ctx, member, *element); };
        Value::Object font{
        ...
        result.emplace_back(observe(*element, ctx, field("cmper", [](const Target& value) { return value.getCmper(); }),
            ...
            field("symbol", [](const Target& value) { return static_cast<std::uint32_t>(value.symbol); }), field("xdisp", &Target::xdisp),
            field("ydisp", &Target::ydisp), field("is_number", &Target::isNumber), field("prefix", &Target::prefix),
...
COVERAGE_CLASS_WITH_PREPARATION(
    "others", "chord_suffix_elements", observeChordSuffixElements, classifyChordSuffixElementDifference, prepareChordSuffixElementComparison);
