...
std::vector<TextChunk> enigmaChunks(const musx::dom::DocumentPtr& document, const std::string& rawText, bool dropTime)
{
    std::vector<TextChunk> result;
    auto insert = [dropTime](const std::vector<std::string>& components) -> std::optional<std::string> {
        if (components.empty()) {
            return std::string{};
        }
        if (dropTime && components.front() == "time") {
            return std::string{};
        }
    ...
    musx::util::EnigmaString::parseEnigmaText(
        ...
        [&](const std::string& text, const musx::util::EnigmaStyles& styles) {
            ...
            if (styles.font) {
                const auto definition = document->getOthers()->get<musx::dom::others::FontDefinition>(musx::dom::SCORE_PARTID, styles.font->fontId);
                if (definition) {
                    charset.emplace(definition->charsetBank, definition->charsetVal);
                }
            }
            result.push_back({text, styles.font, styles.categoryFont, styles.baseline, styles.superscript, styles.tracking, charset});
            return true;
        },
        insert);
    return result;
...
std::optional<std::vector<TextChunk>> tryEnigmaChunks(const musx::dom::DocumentPtr& document, const std::string& rawText, bool dropTime)
{
    try {
...
bool sameFont(const std::shared_ptr<musx::dom::FontInfo>& left, const std::shared_ptr<musx::dom::FontInfo>& right)
{
    if (!left || !right) {
        return left == right;
    }
    try {
        return sameFontName(left->getName(), right->getName()) && left->fontSize == right->fontSize
               && left->getSizeIsPercent() == right->getSizeIsPercent() && left->getEnigmaStyles() == right->getEnigmaStyles();
    } catch (...) {
        return false;
    }
}
...
{
    return sameFont(left.font, right.font) && left.category == right.category && left.baseline == right.baseline
           && left.superscript == right.superscript && left.tracking == right.tracking;
}
...
std::optional<std::string> decodeRawCodepointsAsMacRoman(const std::string& value)
{
    ...
    for (std::size_t at = 0; at < value.size();) {
        ...
        for (std::uint16_t byte = 0; byte <= 0xff; ++byte) {
            ...
            if (value.compare(at, encoded.size(), encoded) != 0) {
                continue;
            }
...
bool symbolFontEncodingGlitch(const std::vector<TextChunk>& source, const std::vector<TextChunk>& companion, bool companionHasExplicitSymbolCharset)
{
    if (source.empty() || source.size() != companion.size()) {
        return false;
    }
    bool affected = false;
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        if (!symbolFont || !converted || *converted != right.text) {
            return false;
        }
        affected = true;
    }
...
std::optional<std::string> reinterpretEncoding(
    const std::string& value, TextChunk::CharsetBank sourceBank, int sourceValue, TextChunk::CharsetBank targetBank, int targetValue)
{
    std::string bytes;
    for (std::size_t at = 0; at < value.size();) {
        bool matched = false;
        for (std::uint16_t byte = 0; byte <= 0xff; ++byte) {
            ...
            if (encoded.empty() || value.compare(at, encoded.size(), encoded) != 0) {
                continue;
            }
            bytes.push_back(raw);
            at += encoded.size();
...
bool isWindowsAnsiReinterpretedAsMacRoman(std::string_view source, std::string_view companion)
{
    ...
    const auto converted = reinterpretEncoding(std::string(source), Bank::Windows, 0, Bank::MacOS, 0);
    return converted && *converted != source && *converted == companion;
}

bool wrongPlatformEncodingGlitch(const std::vector<TextChunk>& source, const std::vector<TextChunk>& companion)
{
    using Bank = TextChunk::CharsetBank;
    if (source.empty() || source.size() != companion.size()) {
        return false;
    ...
    bool affected = false;
    for (std::size_t index = 0; index < source.size(); ++index) {
        if (!sameChunkState(source[index], companion[index])) {
            return false;
        }
        if (!source[index].charset) {
            continue;
        }
        ...
        const auto changed = reinterpretEncoding(source[index].text, bank, value, target->first, target->second);
        if (!changed) {
            return false;
        }
...
bool utf16BytePairGlitch(const std::vector<TextChunk>& source, const std::vector<TextChunk>& companion)
{
    using Bank = TextChunk::CharsetBank;
    if (source.empty() || source.size() != companion.size()) {
        return false;
    ...
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        const auto [bank, value] = *source[index].charset;
        if (value != 0 || (bank != Bank::MacOS && bank != Bank::Windows)) {
            continue;
        }
        ...
        if (pairedCharacters >= 2 && source[index].text == rebuilt) {
            return true;
        }
        ...
        const auto meaningful =
            std::count_if(rebuiltPlain.begin(), rebuiltPlain.end(), [](unsigned char character) { return !std::isspace(character); });
        ...
        if (subsequence && pairedCharacters >= 2 && meaningful >= 8) {
            return true;
        }
    }
    return false;
...
bool chunksEqual(const std::vector<TextChunk>& source, const std::vector<TextChunk>& companion)
{
    if (source.size() != companion.size()) {
        return false;
    }
    for (std::size_t index = 0; index < source.size(); ++index) {
        const auto& left = source[index];
        const auto& right = companion[index];
        if (left.text != right.text || !sameFont(left.font, right.font) || left.baseline != right.baseline || left.superscript != right.superscript
...
std::vector<TextChunk> nonWhitespaceChunks(const std::vector<TextChunk>& chunks)
{
    ...
    for (const auto& chunk : chunks) {
        ...
        while (start < chunk.text.size()) {
            while (start < chunk.text.size() && std::isspace(static_cast<unsigned char>(chunk.text[start]))) {
                ++start;
            }
            auto end = start;
            while (end < chunk.text.size() && !std::isspace(static_cast<unsigned char>(chunk.text[end]))) {
                ++end;
            }
            if (start != end) {
...
bool differsOnlyByWhitespace(const std::string& source, const std::string& companion, const std::vector<TextChunk>& sourceChunks,
    const std::vector<TextChunk>& companionChunks)
{
    ...
    const auto containsWhitespace = [](const std::string& value) {
        return std::any_of(value.begin(), value.end(), [](unsigned char character) { return std::isspace(character); });
    };
...
TextComparison compareText(const std::string& className, const std::string& path, const std::string& source, const std::string& companion,
    ...
    const musx::dom::DocumentPtr& companionDocument, ComparisonResult& result)
{
    ...
    const auto parseTexts = [](const Value::Array& items, const musx::dom::DocumentPtr& document) {
        ...
        for (const auto& item : items) {
            ...
            result.push_back(value && value->isString() ? tryEnigmaChunks(document, value->asString(), false) : std::nullopt);
        }
        return result;
    };
    ...
    for (std::size_t sourceIndex = 0; sourceIndex < sourceItems.size(); ++sourceIndex) {
        if (!sourceChunks[sourceIndex]) {
            continue;
        }
        for (std::size_t companionIndex = 0; companionIndex < companionItems.size(); ++companionIndex) {
            if (matchedCompanion.contains(companionIndex)) {
                continue;
            }
            if (companionChunks[companionIndex] && chunksEqual(*sourceChunks[sourceIndex], *companionChunks[companionIndex])) {
                matchedSource.insert(sourceIndex);
                matchedCompanion.insert(companionIndex);
                ++result.transformations[ComparisonTransformation::SemanticallyPairedPreFinale37BlockText];
    ...
    const musx::dom::DocumentPtr& companionDocument, ComparisonResult& result)
{
    ...
    const auto indicesByNumber = [](Value::Array& items) {
        ...
        for (std::size_t index = 0; index < items.size(); ++index) {
            ...
            if (number && number->isInteger()) {
                result.emplace(number->asInteger(), index);
            }
        }
        return result;
    };
    ...
    const auto rawTextNumber = [](const musx::dom::DocumentPtr& document, musx::dom::Cmper partId,
    ...
    const auto staffNameTexts = [&](const musx::dom::DocumentPtr& document) {
        ...
        for (const auto& staff : sourceInstances<Staff>(document)) {
            for (const auto& nameMember : nameMembers) {
                if (const auto number = rawTextNumber(document, staff->getSourcePartId(), staff.get()->*nameMember.first)) {
    ...
    for (const auto& sourceStaff : sourceInstances<Staff>(sourceDocument)) {
        ...
        const auto companionStaff = companionDocument->getOthers()->get<Staff>(partId, sourceStaff->getCmper());
        if (!companionStaff) {
            continue;
        }
        for (const auto& [member, kind] : nameMembers) {
            const auto sourceText = rawTextNumber(sourceDocument, partId, sourceStaff.get()->*member);
            const auto companionText = rawTextNumber(companionDocument, partId, companionStaff.get()->*member);
            ...
            const auto key = "staff-name-" + std::to_string(partId) + '-' + std::to_string(sourceStaff->getCmper()) + '-' + std::string(kind);
            sourceItems[sourceIndex->second].asObject().insert_or_assign("_report_match_key", Value(key));
            companionItems[companionIndex->second].asObject().insert_or_assign("_report_match_key", Value(key));
            companionItems[companionIndex->second].asObject().insert_or_assign("number", Value(static_cast<std::int64_t>(*sourceText)));
            ++result.transformations[ComparisonTransformation::SemanticallyPairedPreFinale37BlockText];
        }
    }
    ...
    for (const auto number : sourceNameTexts) {
        if (alignedSource.contains(number)) {
            continue;
        }
        if (const auto found = sourceIndices.find(number); found != sourceIndices.end()) {
            sourceItems[found->second].asObject().insert_or_assign("_report_match_key", Value("source-staff-name-" + std::to_string(number)));
        }
    }
    for (const auto number : companionNameTexts) {
        if (alignedCompanion.contains(number)) {
            continue;
        }
        if (const auto found = companionIndices.find(number); found != companionIndices.end()) {
            companionItems[found->second].asObject().insert_or_assign("_report_match_key", Value("companion-staff-name-" + std::to_string(number)));
        }
    }
}
...
{
    static const std::regex pattern(R"(^(?:\^(?:font|fontid|Font|fontMus|fontTxt|fontNum|size|nfx)\([^)]*\))*\^page\([^)]*\)$)");
    ...
    const musx::dom::DocumentPtr& sourceDocument, const musx::dom::DocumentPtr& companionDocument)
{
    ...
    const auto rawText = [](const musx::dom::DocumentPtr& document, const std::shared_ptr<const TextBlock>& block) -> std::optional<std::string> {
        if (block->textId == 0) {
            return std::nullopt;
        }
    ...
    for (const auto& sourceBlock : sourceInstances<TextBlock>(sourceDocument)) {
        if (sourceBlock->textType != TextBlock::TextType::Block) {
            continue;
        }
        const auto key = TextBlockKey{sourceBlock->getSourcePartId(), sourceBlock->getCmper()};
        const auto companion = companionByKey.find(key);
        if (companion == companionByKey.end()) {
            continue;
        }
        const auto prefix =
            "text_blocks[" + partIdentityPrefix(sourceBlock->getSourcePartId()) + "cmper=" + std::to_string(sourceBlock->getCmper()) + ']';
        ...
        if (!sourceText || !companionText) {
            ...
            } else if (sourceBlock->textId != companion->second->textId && sourceBlock->textType != companion->second->textType) {
                result[prefix] = ReferentComparison::Renumbered;
            }
            continue;
        }
        const auto comparison = compareText("block_texts", {}, *sourceText, *companionText, sourceDocument, companionDocument, false, false);
        if (comparison.differences.contains(TextDifferenceClassification::Other)
            ...
            result[prefix] = enigmaTextIsPageInsertOnly(*sourceText) && enigmaTextIsPageInsertOnly(*companionText)
    ...
    const musx::dom::DocumentPtr& sourceDocument, const musx::dom::DocumentPtr& companionDocument)
{
    const bool staffLikePath = path.starts_with("staff[") || path.starts_with("staff_style[")
                               || (path.starts_with("staff_style_assigns[") && path.find("].applied_style.") != std::string_view::npos);
    const bool nameField = staffLikePath && (path.ends_with(".full_name_text_id") || path.ends_with(".abbrv_name_text_id"));
    if (!nameField) {
        return std::nullopt;
    }

    const auto referencedText = [](const musx::dom::DocumentPtr& document, std::int64_t textBlockId) -> std::string {
        using TextBlock = musx::dom::others::TextBlock;
        if (textBlockId == 0) {
            return {};
        }
        const auto block = document->getOthers()->get<TextBlock>(musx::dom::SCORE_PARTID, static_cast<musx::dom::Cmper>(textBlockId));
        if (!block || block->textType != TextBlock::TextType::Block || block->textId == 0) {
            return {};
        }
        const auto text = document->getTexts()->get<musx::dom::texts::BlockText>(block->textId);
        ...
        if (std::regex_replace(text->text, enigmaCommandPattern(), "").empty()) {
            return {};
        }
        const auto chunks = tryEnigmaChunks(document, text->text, false);
        if (chunks && std::ranges::all_of(*chunks, [](const TextChunk& chunk) { return chunk.text.empty(); })) {
            return {};
        }
        return text->text;
    };
    ...
    const auto comparison = compareText("block_texts", std::string(path), sourceText, companionText, sourceDocument, companionDocument, false, false);
...
std::set<std::int64_t> partNameTextIds(const SurveySnapshot& snapshot)
{
    ...
    if (relationships == snapshot.end()) {
        return {};
    }
    const auto* partNames = relationships->second.find("part_names");
    if (!partNames || !partNames->isObject()) {
        return {};
    }
...
{
    if (className != "block_texts") {
        return std::nullopt;
    }
    static const std::regex blockTextPattern(R"(^block_texts\[number=(\d+)\]\.text$)");
    std::smatch match;
    if (!std::regex_match(path, match, blockTextPattern)) {
        return std::nullopt;
    }
...
bool isPartNameText(const std::string& className, const std::string& path, const SurveySnapshot& source, const SurveySnapshot& companion)
{
    ...
    return textId && partNameTextMatches(partNameTextIds(source), partNameTextIds(companion), *textId);
}
...
{
    const auto textId = blockTextNumber(className, path);
    return textId && synthesizedScoreNameText(partNameTextIds(source), partNameTextIds(companion), *textId);
}
    ...
    const musx::dom::DocumentPtr& sourceDocument, const musx::dom::DocumentPtr& companionDocument, bool partNameText, bool synthesizedScoreName)
{
    ...
    const bool removedWhitespaceControl = normalizedSource != source || normalizedCompanion != companion;
    const auto classClassifier = textDifferenceClassifier(className);
    if (classClassifier) {
        if (const auto classified = classClassifier({path, normalizedSource, normalizedCompanion, std::nullopt, std::nullopt,
    ...
    if (!sourceChunks || !companionChunks) {
        std::set<TextDifferenceClassification> differences{TextDifferenceClassification::UnresolvedFont};
        if (removedWhitespaceControl) {
    ...
    if (classClassifier) {
        if (const auto classified = classClassifier({path, normalizedSource, normalizedCompanion, sourcePlain, companionPlain,
                removedWhitespaceControl, partNameText, synthesizedScoreName})) {
            return *classified;
        }
    }
    if (differsOnlyByWhitespace(normalizedSource, normalizedCompanion, *sourceChunks, *companionChunks)) {
        return {false, {TextDifferenceClassification::Whitespace}, {}};
    }
    static const std::regex explicitSymbolCharset(R"(\^(?:font|fontid|Font|fontMus|fontTxt|fontNum)\([^,)]*,(?:8191|8192)\))");
    if (wrongPlatformEncodingGlitch(*sourceChunks, *companionChunks)
        || symbolFontEncodingGlitch(*sourceChunks, *companionChunks, std::regex_search(normalizedCompanion, explicitSymbolCharset))) {
        std::set<TextDifferenceClassification> differences{TextDifferenceClassification::KnownEncodingGlitch};
        if (removedWhitespaceControl) {
            differences.insert(TextDifferenceClassification::Whitespace);
        }
        return {false, std::move(differences), {}};
    ...
    for (std::size_t index = 0; index < common; ++index) {
        ...
        if ((*sourceChunks)[index].font && (*companionChunks)[index].font) {
            ...
            if (std::tie(left->bold, left->italic, left->underline, left->strikeout, left->absolute, left->hidden)
                != std::tie(right->bold, right->italic, right->underline, right->strikeout, right->absolute, right->hidden)) {
                differences.insert(TextDifferenceClassification::Effects);
            }
        }
    }
    if (sourceChunks->size() != companionChunks->size() && differences.contains(TextDifferenceClassification::Other)) {
    ...
    if (std::regex_search(normalizedSource, effectsCommand) != std::regex_search(normalizedCompanion, effectsCommand)) {
        differences.insert(TextDifferenceClassification::Effects);
    }
...
bool hasSynthesizedTextState(const SurveySnapshot& source, const std::string& className, const std::string& path)
{
    ...
    std::smatch match;
    if (!std::regex_search(path, match, numberPattern)) {
        return false;
    }
    const auto classFound = source.find(className);
    if (classFound == source.end() || !classFound->second.isArray()) {
        return false;
    }
    ...
    for (const auto& item : classFound->second.asArray()) {
        ...
        if (!itemNumber || !itemNumber->isInteger() || itemNumber->asInteger() != number) {
            continue;
        }
