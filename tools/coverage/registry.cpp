...
void registerSurveyor(const CoverageClassDescriptor& descriptor)
{
    for (const auto& registered : registry()) {
        if (registered.key == descriptor.key) {
            throw std::logic_error("duplicate coverage surveyor key: " + std::string(descriptor.key));
        }
    }
    registry().push_back({std::string(descriptor.pool), std::string(descriptor.key), descriptor.observe, descriptor.classifyDifference,
...
DifferenceEquivalenceFn differenceEquivalence(std::string_view key)
{
    for (const auto& registered : registry()) {
        if (registered.key == key) {
            return registered.equivalentDifference;
        }
    }
...
{
    for (const auto& registered : registry()) {
        if (registered.prepareComparison) {
            registered.prepareComparison(context);
        }
    }
}
...
TextDifferenceClassifierFn textDifferenceClassifier(std::string_view key)
{
    for (const auto& registered : registry()) {
        if (registered.key == key) {
            return registered.classifyTextDifference;
        }
    }
    throw std::logic_error("unregistered coverage surveyor key: " + std::string(key));
}
...
DifferenceClassifierFn differenceClassifier(std::string_view key)
{
    for (const auto& registered : registry()) {
        if (registered.key == key) {
            return registered.classifyDifference;
        }
    }
    throw std::logic_error("unregistered coverage surveyor key: " + std::string(key));
}
...
SurveyResult runAllSurveyors(const SurveyContext& ctx)
{
    ...
    for (const auto& registered : registry()) {
        ...
        const std::chrono::duration<double, std::milli> elapsed = std::chrono::steady_clock::now() - surveyorStarted;
        result.timings.surveyors.emplace_back(key, elapsed.count());
    }
    const std::chrono::duration<double, std::milli> elapsed = std::chrono::steady_clock::now() - allStarted;
