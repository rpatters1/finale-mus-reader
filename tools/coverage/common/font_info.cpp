...
std::int64_t fontInfoIntegerMember(const Value& object, std::string_view key, std::int64_t fallback = 0)
{
...
std::map<FontInfoObjectKey, const Value*> fontInfoObjectsByPartAndCmper(const Value::Array& items)
{
    ...
    for (const auto& item : items) {
        result.emplace(FontInfoObjectKey{fontInfoIntegerMember(item, "part_id"), fontInfoIntegerMember(item, "cmper")}, &item);
    }
    return result;
}

const Value* fontInfoObjectForPart(const std::map<FontInfoObjectKey, const Value*>& objects, std::int64_t partId, std::int64_t cmper)
{
...
std::set<std::string> comparisonFontReferencePaths(const SurveySnapshot& snapshot)
{
    ...
    if (shapesFound == snapshot.end() || !shapesFound->second.isArray() || listsFound == snapshot.end()) {
        return result;
    }
    const auto* listsValue = listsFound->second.find("lists");
    if (!listsValue || !listsValue->isArray()) {
        return result;
    }
    ...
    for (const auto& shape : shapesFound->second.asArray()) {
        ...
        const auto* list = fontInfoObjectForPart(lists, partId, fontInfoIntegerMember(shape, "instruction_list"));
        if (!list) {
            continue;
        }
        ...
        for (const auto& instruction : instructions->asArray()) {
            if (fontInfoIntegerMember(instruction, "type") == 20) {
                result.insert("shape_data.buffers[" + partIdentityPrefix(bufferPartId) + "cmper=" + std::to_string(dataCmper) + "].values["
...
std::string comparisonFontIdentity(const SurveySnapshot& snapshot, std::int64_t id)
{
    ...
    for (const auto& definition : definitions->asArray()) {
        if (fontInfoIntegerMember(definition, "cmper", -1) != id) {
            continue;
        }
...
bool isComparisonFontReference(std::string_view path, const std::set<std::string>& shapeFontPaths)
{
    ...
    const auto endsWith = [path](
                              std::string_view suffix) { return path.size() >= suffix.size() && path.substr(path.size() - suffix.size()) == suffix; };
    return endsWith(namedReferenceSuffix) || endsWith(nestedFontInfoSuffix) || shapeFontPaths.contains(std::string(path));
}
